...
describe('assembleAgentPrompt coding routing policy', () => {
  it('renders coordinator-first routing instead of main-direct coding as default', () => {
    ...
    expect(prompt).toContain('spec_reviewer=APPROVED 且 code_quality_reviewer=APPROVED')
    ...
    expect(prompt).toContain('仅 code_quality_reviewer 的 minor / nit')
    expect(prompt).toContain('split reviewer 缺少 STATUS')
    expect(prompt).toContain('spec 缺少 MISSING / EXTRA')
    expect(prompt).toContain('quality 缺少 CRITICAL / IMPORTANT / NIT')
