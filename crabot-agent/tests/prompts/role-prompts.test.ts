...
describe('独立角色提示词', () => {
  it('builtin 明确委托关系并保留执行闭环，不继承主控人格或模型槽位', () => {
    ...
    for (const absent of ['PERSONALITY_SENTINEL', 'Manager', 'powerful', 'cost_effective', 'send_message', 'get_subagent_output', 'set_task_goal']) expect(prompt).not.toContain(absent)
    expect(prompt).toContain('接受主控（调用方）委托')
  ...
  it('主控保留及时响应的理由和同项目上下文复用条件', () => {
    ...
    expect(prompt).toContain('范围明确的简单任务直接交给内置执行器')
    expect(prompt).toContain('可独立推进的工作可以同时安排多个执行器')
    expect(prompt).toContain('继承已有上下文并行处理新请求时建立执行分支')
