...
describe('TraceStore manager episode traces', () => {
  ...
  it('persists bounded redacted message history, deduplicates recovery and retains original trigger', () => {
    const writer = store.managerTraceWriter(text => text.replaceAll('secret-value', '[REDACTED]'))
    writer.startEpisode('inputs', KEY_A, TRIGGER)
