...
// ---------------------------------------------------------------------------
// Output tool span emission
// ---------------------------------------------------------------------------

describe('Output tool with traceContext', () => {
  it('emits bg_entity_output span when reading transient shell output', async () => {
    const { createOutputTool } = await import('../../../src/engine/tools/output-tool')
    const store = makeMockTraceStore()
    const traceContext = makeTraceCtx(store)
    const transient = new TransientShellRegistry()

    const entityId = transient.spawn({
      command: 'echo hello_output_test',
      owner: { friend_id: 'user-C' },
    ...
    for (let i = 0; i < 20; i++) {
      await sleep(50)
      const state = transient.get(entityId)
      if (state && state.ringBuffer.includes('hello_output_test')) break
    }
    ...
    await tool.call({ entity_id: entityId }, { abortSignal: new AbortController().signal })

    expect(store.startSpan).toHaveBeenCalledOnce()
    const call = store.startSpan.mock.calls[0]
    expect(call[1].type).toBe('bg_entity_output')
    const details = call[1].details as Record<string, unknown>
    expect(details.entity_id).toBe(entityId)
    expect(typeof details.bytes_read).toBe('number')
    expect((details.bytes_read as number)).toBeGreaterThan(0)
    expect(details.type).toBe('shell')
  })

  it('does not emit span when traceContext is undefined', async () => {
    const { createOutputTool } = await import('../../../src/engine/tools/output-tool')
    const store = makeMockTraceStore()
    const transient = new TransientShellRegistry()
...
// ---------------------------------------------------------------------------
// Kill tool span emission
// ---------------------------------------------------------------------------

describe('Kill tool with traceContext', () => {
  it('emits bg_entity_kill span when killing a running transient shell', async () => {
    const { createKillTool } = await import('../../../src/engine/tools/kill-tool')
    const store = makeMockTraceStore()
    const traceContext = makeTraceCtx(store)
    const transient = new TransientShellRegistry()
    ...
    const entityId = transient.spawn({
      command: 'sleep 10',
      owner: { friend_id: 'user-D' },
      spawned_by_task_id: 'task-10',
    })

    const tool = createKillTool({
      registry,
      transient,
      cursorMap: new Map(),
      taskId: 'task-10',
    ...
    expect(store.startSpan).toHaveBeenCalledOnce()
    const call = store.startSpan.mock.calls[0]
    expect(call[1].type).toBe('bg_entity_kill')
    const details = call[1].details as Record<string, unknown>
    expect(details.entity_id).toBe(entityId)
    expect(details.status_before).toBe('running')
  })

  it('does not emit kill span when traceContext is undefined', async () => {
    const { createKillTool } = await import('../../../src/engine/tools/kill-tool')
    const store = makeMockTraceStore()
    const transient = new TransientShellRegistry()
