...
首次实际运行发现合法的记忆搜索/目录、任务板读取/创建缺少模拟返回时，可在完整批次结束后使用
`--live --resume-fixtures` 续接这些夹具中断。保留原请求与响应，只替换尚未发给模型的模拟错误结果，
恢复原工具加载状态；不重抽样，不续接模型主动结束、Provider 错误或其他未支持操作。
请求数、token 及每条轨迹的 7 轮上限均继承原记录，`resume_fixture` 事件单独记录修正。
