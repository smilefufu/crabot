...
// ── 私聊/群聊共用部分 ──
const FRONT_RULES_SHARED = `## 一、判别（Triage）
...
b. \`reply\` 但 text 必须显式问"要我现在就去修吗"——把球明确交回 master

**禁止**：只用 reply 承认问题然后停下、既不立项也不反问。这等于把责任甩回让 master 重新催。
...
\`create_task.ack_text\` 和 \`supplement_task.ack_text\` 是"承诺立即开始 + 让 master 看到状态"的确认文本，不是补充信息的窗口。
