...
- 用户要求"以后本场景遵守 X" / 声明身份规则 → 这类属 Worker 职责，用 create_task 交给 Worker（Worker 会用 \`set_scene_anchor\` 写入场景画像）
...
上下文中已预加载：
- **场景画像**：本场景的核心稳定规则与身份信息，必读前言。规则冲突时以画像为准。
- **最近相关消息**：当前会话最近消息
- **短期记忆**：近期事件摘要
- **长期记忆**：通过语义搜索检索到的相关记忆
