...
export function assembleDispatcherPrompt(ctx: DispatchContext): string {
  const hasSupplementCandidates = ctx.activeTasks.length > 0
  const parts: string[] = []
  parts.push(PRODUCT_SELF)
  parts.push('')
  // 群聊才有收件人歧义（多人 + 多 bot 共存）；私聊/admin chat 只有两方，不需要这段
  if (ctx.sessionType === 'group') {
    parts.push(buildGroupRecipientPreamble(hasSupplementCandidates))
    parts.push('')
  }
  parts.push(buildDispatchRules(ctx.sessionType, hasSupplementCandidates))
  parts.push('')
  ...
  if (ctx.sessionType === 'group') {
    parts.push(buildSystemEventGuidance(hasSupplementCandidates))
    parts.push('')
  }
  parts.push(buildOutputSchema(ctx.sessionType, hasSupplementCandidates))
  return parts.join('\n')
}
...
你不直接与人类对话——你的判断会让系统调起相应的 agent 实例执行任务，或把消息作为补充投递给可补充任务，或者忽略（仅群聊）。`

function buildGroupRecipientPreamble(hasSupplementCandidates: boolean): string {
  const allowedActions = hasSupplementCandidates ? 'new_task / supplement / stay_silent' : 'new_task / stay_silent'
  const recipientAction = hasSupplementCandidates ? 'new_task / supplement' : 'new_task'
  return `## 分诊前置：先判收件人（最高优先级）

判断 ${allowedActions} 之前，先逐条判断这条消息发给谁。**收件人判定优先于“消息里有动词/任务/服务器/代码”等任务特征。**

- 明确发给我的信号：消息带 mention="@you"，或正文出现「你在本渠道的身份」段里我的 @handle，或当前消息明确在追问我刚才发出的内容。满足才可 ${recipientAction}。
...
判断要点：
- 因为没有可补充任务，所有消息都必然是新任务——不存在"补充某个已有任务"的可能性
