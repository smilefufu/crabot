...
**Bash 有 10s 前台宽限期**：命令在 10s 内完成就正常同步返回；**超过 10s 仍在跑会自动转入后台（命令不中断）并返回 entity_id**——你不用预先判断时长、也没有什么"后台开关"要打开，长命令照常直接跑即可，不会堵住 agent loop。收到「已转入后台」后：有别的事就去做；没有其他工作时自然结束当前回合，命令退出事件会唤醒你。**不要对转后台的命令反复裸 poll \`Output\`。**
