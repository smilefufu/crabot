...
function emptyObjectiveReminder(objective: Pick<WorkboardObjective, 'objective_id' | 'title'> | undefined) {
  return objective ? {
    system_reminder: {
      ...objective,
      message: `该目标已无当前事项。请结合最新的人类要求、已有结果及你最近对人类的答复，核实目标是否达成，并检查是否仍有未落实的后续工作。已达成或明确不再继续的目标，按实际结果归档；阶段目标完成不等于整体要求完成，仍需持续管理的工作保留在当前任务板中。

对你已提出的“下一步”，区分执行承诺与可选建议：已经承诺继续，或属于人类已要求且现有授权覆盖的工作，能够推进时应落实执行安排，不能只描述下一步便结束，也不要默认等待人类再次催促；确实受阻时保留具体阻塞。只是超出当前要求的可选建议，或确需人类作出新决定时，可以等待，但应向人类说清是在提出建议或等待决定，不把沉默当作同意。

需要向人类汇报或讨论的，按需沟通。不要仅因事项清空就认定目标完成，不为填补空目标而创建事项，不重复安排已有执行，也不为本次检查重复发送消息。`,
    },
  } : {}
}
