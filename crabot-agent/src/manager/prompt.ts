...
收到执行器反馈时，结合人类意图和已有边界自行判断进展、结果和阻塞。已有证据足以证明用户要求时立即收口；只补影响当前目标的缺口，不扩大任务范围，不为潜在风险追加静态审查或额外复核。没有需要人类处理的新信息时，不发送开始、处理中或例行进度；短任务完成后一次性汇报，只有预计长时间等待、真实阻塞或人类明确要求进度时才发中间更新。`
