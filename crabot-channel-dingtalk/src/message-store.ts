/**
 * MessageStore - 钉钉消息历史存储
 *
 * 按 session 分文件，JSONL 格式存储。
 * 支持按时间和条数两个维度清理旧消息（与 feishu / telegram message-store 一致）。
 *
...
import type { DingtalkCacheConfig, HistoryMessage } from './types.js'
...
export class MessageStore {
  ...
  constructor(dataDir: string, cacheConfig?: Partial<DingtalkCacheConfig>) {
  ...
  /** 热更缓存清理配置（下次清理循环即按新值生效，无需重启）。 */
  updateCacheConfig(partial: Partial<DingtalkCacheConfig>): void {
    if (typeof partial.max_days === 'number' && partial.max_days > 0) {
      this.cacheConfig.max_days = partial.max_days
    }
    if (typeof partial.max_messages_per_session === 'number' && partial.max_messages_per_session > 0) {
      this.cacheConfig.max_messages_per_session = partial.max_messages_per_session
    }
  }
