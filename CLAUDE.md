...
- 普通小改动不主动增加简化步骤。对经历多步实现或多轮修改的较长编码任务，在实现完成并通过首轮定向验证后、最终 Code Review 前，主动检查本次 diff 是否存在明显的复杂嵌套、重复逻辑、临时拼接结构或冗余抽象。
- 发现明确的简化机会时，若当前环境提供 `code-simplifier`，使用它整理本次改动；未安装或不可用时直接跳过，不为此安装 skill，也不阻塞任务。代码已经清晰时同样跳过。
- 简化只限于本次任务修改的代码，必须保持行为和契约不变，不得顺手整理相邻旧代码或引入新的设计决策。项目规则、正式协议和当前模块的既有风格优先于 skill 的通用偏好。
- 简化产生代码修改后，必须重跑相关验证。最终 Code Review 审查整理后的 diff；代码简化不能替代 Code Review。
...
设计型任务的 PR 有 @claude auto review，需定期跟踪其意见。**review 意见不是工单——先定性、后动手，严禁照单全收**：

- 每条意见先回复**定性结论 + 证据**，再决定是否改代码：
  - `[真实风险]`：当前生产条件可达、违反协议/安全/正确性 → 修
  - `[理论风险]`：当前量级/条件下不可达 → 不改，回复说明理由并记 follow-up
  - `[风格/口味]` → 不改，回复讨论或拒绝
  - 定性拿不准 → 停下来问，不得擅自修
- 修复提交只允许触达该意见所指的行/文件。若正确修法需要更大的重构（改生命周期、所有权、持久化模型），**不得并入修复**——记 follow-up 或走 spec 流程
- 每次 push 前自己先通读完整 diff，并在提交说明里写清两件事：① 每块改动对应哪条需求 / 哪条 review 意见 / 哪个实测证据（没有出处的改动不许提交）；② 本次改动新引入了哪些失败路径（新增的超时、目录、状态、进程等），各自如何收口
- review 意见指向设计语义变更（生命周期/所有权/持久化模型等）时，**必须停下来要求人类决策**——不得自行改 spec 或写代码
