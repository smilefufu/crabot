...
- 原 spec §7.2 修订已确认并同步协议：daily 退出前只读校验 schema、单独调用及完整已读的本周期引用；拒绝整批并返回错误，模型在原轮次预算内纠正。合法 partial 不受全局缺口阻挡，退出后最终校验保留。新增回归先失败后通过，Engine/宿主/Manager trace 等 361 项及 Agent 构建通过；扩展 registry 检查 77 项通过、2 项 Memory 错误夹具在未修改 main 上同样失败，独立记录不扩大修复。待 PR 自动合并、部署及当前实例单次实际验收；历史缺口、水位和自动计划暂停状态不变。
