...
- 合入最新 main 并解决工具装配冲突，确认失败的 trace 产品结果改为 partial，保留原确认重发。最新 Agent 505 项定向验证、Agent/Admin 构建通过；Core 172 项、Shared 111 项、CLI 193 项全量通过。Agent/Admin 全量中的旧夹具断言及旧重试测试挂起已与 main 对照，不宣称全量通过。Memory 纠正参数后旧失败仍阻止完成的问题尚未解决，不以无关成功清空失败，也未加入可忽略失败的新接口。
