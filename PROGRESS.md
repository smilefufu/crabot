...
### 主控与执行器内置 Guidance：基础版已交付，完整执行重验进行中
...
- 用户要求重做完整验证。旧评测首次业务选择即停止，无法覆盖“查板→读结果→错误外发”的事故后半段，不能证明质量或性能未回退。已新增真实 Manager/Harness/Builtin 完整循环和 Docker 项目/Git/规则/子审查传输，21 项评测器回归通过；冻结 12 类、34 条轨迹，上限 720 请求/200 万 tokens。用户进一步明确授权本 session 此类验证，首批 118 请求/2,011,190 tokens：14 条完整收口、1 条预算中断、19 条未启动，接续原计划未启动项；本轮不再重复索要授权。判据见 `crabot-agent/eval/guidance/CONTINUOUS-ACCEPTANCE.md`；产品正文未改。
