...
- production `capabilityBundle` 在每次 spawn/handoff 现取当前 `agentConfig.mcp_servers`，用 `BUILTIN_WORKER_PERMISSIONS ∩ principal_permissions` 收敛后按 server 过滤：`computer-use→desktop`，其余（含 Git MCP）→`mcp_skill`；CLI 物化保留 stdio `env`、远端 `headers/http_headers` 与 Claude 远端 transport `type`，避免需要凭证的 server 被静默下发成半配置。Claude spawn/resume/fork 均使用 `--mcp-config .mcp.json --strict-mcp-config`，不再与宿主 user/local MCP 求并集；Claude `.mcp.json` 与 Codex 隔离 `config.toml` 均原子替换并收紧为 `0600`。Claude provision 在任何其他写入前拒绝覆盖 Git 已跟踪的 `.mcp.json`；未跟踪目标文件及原子写 crash temp 则保留既有 `.gitignore` 并幂等追加 `/.mcp.json`、`/..mcp.json.tmp-*`，防止普通 `git add -A` 误提交凭据。skills 仍为空，manager 工具面不变。
- 验证：review-fix 定向 **6 files / 168 passed**，随后 Claude/Codex/物化安全修订全量 **3 files / 171 passed**，`.mcp.json` Git 排除/已跟踪拒绝修订 **2 files / 97 passed**，crash temp 排除聚焦 **2 passed**；workers/harness + manager + provision + MCP 回归 **31 files / 605 passed**（`TMPDIR=/private/tmp`、串行）；Agent 全量基线 **2623 passed / 1 failed / 2 skipped**，唯一失败仍为未改动 `tmux-driver.test.ts` 的已知 foreground-command 环境差异（期待 `sleep`、实际 `bash`，隔离重跑复现）。Agent TypeScript build 与 `git diff --check` 通过，未生成 `crabot-agent/package-lock.json`。权限/恢复 fresh review 无 material finding；production callback→真实 Harness spawn/handoff→目标 provision 组合证据及 builtin→CLI 方向已补齐，follow-up review 为 0 blocker。GitHub 自动 review 首轮指出的 MCP 凭证物化与旧部分权限兼容问题已在 `28cc236` 修复；后续复审提出的宿主 MCP union、凭据文件 mode、目标/临时文件误提交风险均已修复并补回归；未来 schema 升级项按协议“persisted read 严格、损坏 fail-loud”不在本次静默归一化，新增权限类目前必须先做显式 context migration。待下一轮自动复审/merge。
