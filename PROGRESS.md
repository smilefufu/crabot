...
### 主控与执行器内置 Guidance：完整任务比较已有结果，整体验收未通过

- 已实现[确认方案](crabot-docs/superpowers/specs/2026-09-16-builtin-guidance-and-core-prompts-design.md)的 269/162 字核心、主控 4 份/执行侧 3 份独立工作流、每日反思专用正文、执行条件查询及明确授权的主执行器桌面能力。旧产品项目维护 Skill 迁出，同名用户 Skill 保留；此前自动 guidance 装配错误，不能称为整体方案已通过验收。
...
- 用户已关闭提前创建的 draft PR #166。仅本地修复与比较，不重开、不新建 PR、不推送或部署；完整任务质量验收尚未完成。不得创建 draft PR。
...
- 用户确认后仅调整反馈与诊断指南：有效证据复用、保留必要验证、未知不算通过。205 项检查、2 项 Docker 反例及构建通过。具体外发范围获明确“授权”后，冻结对照运行 89 次请求、1,424,398 tokens；5 条正常收口、1 条交付后内部收尾被预算截断、2 条原简单诊断未开始。两个正常完成配对 tokens 少 20.6%，但执行器业务调用 18→21，不能认定重复取证改善；历史摘要缺原生轨迹也限制解释。保留本地候选，不推送/提 PR/部署，详细质量、动作及用量见 `crabot-agent/eval/guidance/EVIDENCE-RESULTS.md`。
...
- 既有全量测试曾中止，基线复查重现 9 项已有失败，未将其余失败全部归因；本轮只报告上述相关测试范围。完整任务比较中的固定 system 保持不变，缓存字段齐全；渠道和记忆仍是本地边界，代表性业务分布、全部指南和权限组合尚未验收。
