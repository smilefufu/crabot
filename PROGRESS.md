...
### 主控与执行器内置 Guidance：自动装配缺陷待修，方案验收撤回

- 已实现[确认方案](crabot-docs/superpowers/specs/2026-09-16-builtin-guidance-and-core-prompts-design.md)的 269/162 字核心、主控 4 份/执行侧 3 份独立工作流、每日反思专用正文、执行条件查询及明确授权的主执行器桌面能力。旧产品项目维护 Skill 迁出，同名用户 Skill 保留；自动 guidance 的装配位置不符合既有协议，不能称为完整实现已通过验收。
- 用户指出缓存问题后确认：Worker 反馈/任务板指南被动态拼入 system prompt，场景变化也改变缓存标识。此前测试遗漏固定前缀不变量，最新四条主控轨迹、历史主控续办及早期事件模拟使用错误装配；质量与成本汇总不得为目标方案背书。撤回“基础能力保留、未观察新增退步”的方案判断，原数据保留；暂停模型比较，先修装配并做本地验证。当前尚未修复或部署。
- 用户明确子 Agent 隔离仅限 Crabot 内置子 Agent；已验证其不接收主线核心、guidance 和新增桌面能力。Claude Code/Codex 原生子 Agent 沿用原生继承，不另加隔离机制。
- 本地 Agent、Admin、正文装配与权限定向检查通过，CLI 环境跳过与基线已有的 Claude 监听失败单独记录。评测模拟器新增 3 项回归通过，生产核心及 7 份工作流沿用确认稿，未因模型表现追加措辞。
- 用户授权后完成第二轮 6 组双版本、12 条轨迹、33 次请求。翻译、禁止派发、桌面委派方向正常，新版一条脚本修复完成模拟核验与收尾；3 条轨迹仍有模拟工具缺口，错误场景权限回执矛盾的两条不计安全结论。修复夹具不回写旧结果，不宣称整体改善或完整行为验收通过。
- 按用户要求追加 Docker 真实执行：2 组双版本、4 条轨迹、19 次请求，无模拟命令回执。两版均修复脚本并通过独立 6 组验算，只读诊断也均正确且文件不变；新版修复少 1 次请求，但诊断 6 次请求/11 次工具，旧版 4/3，流程和表达偏重。真实工具与权限的 3 项定向检查通过；本轮未重跑主控模型与完整调度，不宣称整体改善。
- 历史问题补充完成：3 类、新旧各两次，含用户重置额度后恢复首条，共 188 次模型请求。主要目标旧版 6 条完成（含恢复）、新版 5 条；新版一次将输入方式固化成排他限制且预算耗尽未交付。已返回 tokens 旧版 1,251,921、新版 1,179,895（少 5.8%，非同质量效率结论）。容器并发 Read 达到进程上限及主控本地边界缺口单独记录；4 项本地链路检查通过。核心与 guidance 未改。
- 同步 main 后更新旧测试与确定性任务板脚本，相关 313 项及真实 tmux 的 3 项通过，Agent 构建通过。全量未通过且运行中止；基线复查重现 9 项既有失败，未把其余失败全部归因或扩围修复。历史完整链模型比较仍对应合并前版本。
- 后续先复现并修复评测器预算取消已准入响应、Docker 并发进程上限两项干扰；9 项本地链路通过，旧结果不回写。新增短决策测试器 8 项通过；生产提示词不改。
- 按用户同意的分阶段验收，在合并后产品 `59112e91` 冻结 10 例、新旧各两次，实际 55 次请求/224,127 tokens、无重试。14 对同样符合、2 对都有仅需清单却先选 Shell 的问题、4 对证据不足；新版 tokens 少 13.0%，输出和未缓存输入反而增加，不宣称实际费用更低、任务成功率提升或完整质量不退步。
- 以草稿 PR 交付，未合并、未部署。完整结果见 `crabot-agent/eval/guidance/RESULTS.md`，过程及既有测试记录见[实施计划](crabot-docs/superpowers/plans/2026-09-17-builtin-guidance-implementation.md)。
