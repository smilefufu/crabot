...
### Manager 交接提示词：多案例验证完成，detailed 出现局部改善

- 修正目标 Worker、redirect、候选插入位置和重试控制后，4 案 × 3 次 × 3 条件得到 36 条完整交接；37 次请求含 1 次空响应超时重试，全部记录保留、无完整输出补抽、无真实工具执行。
- 先隐藏条件评阅再揭示标签：detailed 在原权限案例 3 次均更好地消化反问并保住调查边界，视频 1 次轻微优势，其他比较接近；精简版未见额外收益。正向集中于同一案例，尚未证明泛化，探索批的越界反例仍保留。
- 13 项本地测试、冻结材料及实际响应审计通过；生产提示词未改，私有材料仅保留在本地忽略目录。后续优先沿 detailed 扩展不同背景的真实坏交接，继续检查范围保真。
