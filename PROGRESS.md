...
### 主控与执行器内置 Guidance：实现与 Docker 定向比较完成，草稿 PR 待审阅
...
- 同步 main 后更新旧测试与确定性任务板脚本，相关 313 项及真实 tmux 的 3 项通过，Agent 构建通过。全量未通过且运行中止；基线复查重现 9 项既有失败，未把其余失败全部归因或扩围修复。模型比较仍对应合并前产品版本。
