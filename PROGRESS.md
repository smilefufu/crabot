...
### 主控与执行器内置 Guidance：基础版已交付，规范与工作流归位待行为验证
...
- 外部真实模型对照准备为 18 案 × 旧新两版，36 条轨迹，最多 144 请求/50 万 tokens。自动审批因缺少本次具体内容与百炼目的地的明确外发授权而拒绝，已询问用户；当前零请求，行为结论待测。范围与限制见 `crabot-agent/eval/guidance/BOUNDARY-RESULTS.md`。本次增量未推送、未建 PR、未部署。
