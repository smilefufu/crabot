...
- 补齐既有历史 inbox 人工迁移边界：可信每日反思每次确认、删除、更新或建链前核实目标当前状态，缺少 `inbox_entered_at` 的 inbox 拒绝修改，查询失败同样不写；普通主控、Admin 和正常新候选行为不变。不增加完成门槛或失败账本。回归先失败后通过，Agent 150 项定向测试与构建通过；原 6 次确认和 14 次删除的 Engine 重放全部保留失败回包，20 条真实记录的只读代入也全部拒绝、零写入。真实模型的候选取证、工具族加载与收口仍待验收，自动计划继续暂停。
- 223 条旧消息使单次输入约 32–35 万 token；当前模型窗口为 1,048,576，原 spec 的 80% 压缩阈值为 838,860，本次未触发不构成压缩实现失效，也不能据此宣称总体 token 节省已验收。未改阈值、清空历史或重置水位。
