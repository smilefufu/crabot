...
### 主控与执行器内置 Guidance：基础版已交付，增量重验发现权限问题

- 基础版已完成正式审查和本地部署；普通主控 4 份、主执行侧 3 份产品工作流与用户 Skill 独立，每日反思直接装配，内置子 Agent 保持隔离。完整旧批次证据见 `crabot-agent/eval/guidance/COMPLETION-RESULTS.md`、`EVIDENCE-RESULTS.md`；这些结果不证明整体效率提升，错误装配批次的结论保持撤回。
- 2026-09-18 根据已确认的全量审查调整两个核心与七份指南：任务板概念和维护规范常驻主控核心，自省独立；项目流程按阶段进入；执行器反馈区分最终交付、子任务和内部检查。主控/执行侧核心为 298/191 个非空白字符，新增[增量设计](crabot-docs/superpowers/specs/2026-09-18-guidance-workflow-boundaries-design.md)。
- 普通改板不再先返回指南而跳过写入；管理员更新只保留原短提示，自省和需处理的 Worker 反馈继续使用 episode 尾部临时指南。未改缓存算法、任务板权限/read fence、每日反思、内置子 Agent 或原生三方继承。
- 5 项问题在旧源码先复现；修复后去重计 16 个文件 442 项定向回归、17 项 Docker 真实工具链检查及 12 项评测器检查通过，Agent 构建、正文与设计逐字核对及本次 diff review 通过。Docker 模型为脚本响应，不能当作真实模型行为验收。
- 用户明确授权具体外发后，冻结的 18 案 × 旧新两版完成 36 条短决策轨迹：40 请求、253,310 tokens，无接口错误、截断、重试或补样。新版总 tokens 少 3.64%，输出多 6.66%；缓存字段齐全，固定 system 未变，但未缓存差主要来自首请求缓存状态，不能认定成本更低。
- 自省沉默、必要提问、内部取证/子任务/最终交付等已观察选择符合主要边界，旧版同案未复现线上泄露。新版权限不足案例先建目标并加入未要求的验收条件；旧版错误推断执行器数量为 0。该例不能验收为通过；组织、协作、诊断三份正文未进入后续模型决策，不声称七份流程全面验收。详见 `crabot-agent/eval/guidance/BOUNDARY-RESULTS.md`。本次增量未推送、未建 PR、未部署。
- 完整执行重验已结束：冻结 12 类、34 条轨迹，真实 Manager/Harness/Builtin 循环，Docker 内实际文件、命令、Git 和内置 reviewer；两批合计 251 请求、3,371,142 tokens，33 条运行结束、1 条预算中断，未重跑或补抽失败。两版七份指南正文均进入后续请求，但不等于全部行为通过。用户已持续授权本 session 此类验证。
- 新版两次权限样本分别多要已有 file_io/shell，以及收到 can_spawn=false 后仍尝试派发、遭拒才提问；门禁有效、文件未动。旧版一次将采购约定泛化写入 inbox 记忆候选。三轮暂停自省两版都未外发，不能证明旧事故已修复。剔除传输夹具缺口与不合格/中断项后的 12 对，tokens 少 4.08%、输出多 5.19%、耗时基本持平；具体任务存在用量上升，不宣称质量或性能无回退。详见 `crabot-agent/eval/guidance/CONTINUOUS-RESULTS.md`。
- 补齐人工渠道只读传输；慢速子审查回归复现评测器提前结束，再修正等待 running/待投递状态，连续运行 11 项回归通过。本批两条真实协作轨迹均有审查回传和父任务最终收口，未受该缺陷截断。两版编译摘要与冻结包复核一致，产品正文未改；增量仍未推送、PR 或部署。
