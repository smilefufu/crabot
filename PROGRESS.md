...
- 只读列表复用自主对账的项目身份快照，范围变化或新 workspace 未观测时保留待核实；实际淘汰仍使用新鲜核验。重复轮询零新增 Git 解析等 230 项通过；合入同期 main 后定向 234 项通过、5 项 tmux 跳过，Agent 与 ES2022 Web 类型检查通过。全量未全绿：Web 12 项 localStorage 失败在干净 main 复现；Agent 存在基线失败、tmux 环境波动，两边均因既有重试假时钟用例阻塞而中断。CLI 历史子 Agent 状态的运行/停止取证边界待用户确认。
