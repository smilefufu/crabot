...
- 审查修复：已处置为 closed 的零 turn 分析 Worker 不再永久 pending；反思临时游标不淘汰普通会话游标；Manager 证据读取包含被内存淘汰的磁盘归档。失败清点不发布半成品目录，已发布目录的详情只按原定位和上界重试。
- 最新 Agent 491 项定向验证及构建通过；此前 Admin 68 项和敏感 RPC 11 项通过。Memory 纠正参数后旧失败仍阻止完成的问题尚未解决，不以无关成功清空失败，也未加入可忽略失败的新接口。
- 提示词与 guidance 由指定任务负责；上线前核对完整装配，部署后以真实 daily trace 和水位对账验收。源码/测试通过不代表已部署或历史缺口已补齐。
...
### 主控与执行器内置 Guidance：方案与正文定稿，进入正式 PR 审查

- 已实现[确认方案](crabot-docs/superpowers/specs/2026-09-16-builtin-guidance-and-core-prompts-design.md)的 269/162 字核心、主控 4 份/执行侧 3 份独立工作流、每日反思专用正文、执行条件查询及明确授权的主执行器桌面能力。旧产品项目维护 Skill 迁出，同名用户 Skill 保留。此前自动 guidance 装配错误已修复；结构验收与模型行为改善分别记录。
- Worker 反馈/任务板指南已从动态 system 改为当前 episode 的尾部临时消息：固定配置下 system 与缓存键不变，运行中注入不改写已有消息，恢复后补回一次，正常/失败/压缩路径均不残留到后续历史；缓存键算法和确认稿正文未改。最初 5 项装配断言先失败再通过，压缩后失败的临时消息残留也已复现并修复。
- 相关 40 个测试文件共 741 项通过，决策测试器 9 项、Docker 真实工具链 11 项通过，Agent 构建通过；Docker 使用脚本化模型响应，无外部模型调用。比较脚本同步改用尾部消息，基线组排除新增自动正文。旧模型数据及撤回结论保持不变。
- 用户已关闭提前创建的 draft PR #166；现已确认提示词语义定稿，并授权建立正式 PR、跟进 auto review 至自动合并，再更新本地部署。不得创建 draft PR，不自行合并。
- 修复版 `ee4c5c6d` 的短决策比较共 40 条、57 次请求。后续补齐主控 Docker 文档/本地记忆边界，17 项完整链测试及 9 项决策测试通过，再做固定产品版本的完整任务比较：286 次请求、3,500,972 tokens，29 条开始、28 条完整收口；一条预算中断、三条未开始，无重试或补样。
- 13 个完整配对均达成实际目标；旧版三次实质错误汇报、新版未出现同类缺陷，但新版诊断业务调用从 10 增至 19、该例 tokens 多 60.8%，全体配对 tokens 多 12.7%。支持主控汇报准确性的有限正向证据，不支持整体质量/效率胜出；该批未改变核心或 guidance，不以重复取样稀释已见过程问题。
- 用户确认后仅调整反馈与诊断指南：有效证据复用、保留必要验证、未知不算通过。205 项检查、2 项 Docker 反例及构建通过。具体外发范围获明确“授权”后，冻结对照运行 89 次请求、1,424,398 tokens；5 条正常收口、1 条交付后内部收尾被预算截断、2 条原简单诊断未开始。两个正常完成配对 tokens 少 20.6%，但执行器业务调用 18→21，不能认定重复取证改善；历史摘要缺原生轨迹也限制解释。保留已确认候选，不将本轮结果表述为解决重复取证或整体效率提升；详细质量、动作及用量见 `crabot-agent/eval/guidance/EVIDENCE-RESULTS.md`。
- 用户明确子 Agent 隔离仅限 Crabot 内置子 Agent；已验证其不接收主线核心、guidance 和新增桌面能力。Claude Code/Codex 原生子 Agent 沿用原生继承，不另加隔离机制。
- 历史实验记录保留：早期模拟存在覆盖缺口；Worker Docker 比较 19 次请求，两版完成，但新版诊断 6 次请求/11 次工具，旧版 4/3；历史完整链共 188 次请求，主要目标旧版 6/6、新版 5/6；短决策比较 55 次请求。主控使用错误装配，13.0% / 5.8% token 差异不再作为目标方案证据，不回写数据或把失败改判成功。
- 正式 PR 前 256 文件 3356 项回归首轮 3336 通过、20 失败；两个旧文案断言修正后 builtin 文件 86/86 通过，其余 18 项在当前 main 复现。另有两边都出现的 CLI 监听间歇失败。Admin 45 项及两端构建通过，未将既有失败算作通过。完整任务比较中的固定 system 保持不变，缓存字段齐全；渠道和记忆仍是本地边界，代表性业务分布、全部指南和权限组合尚未验收。
- 完整任务证据见 `crabot-agent/eval/guidance/COMPLETION-RESULTS.md`，旧批次见 `RESULTS.md`；[实施计划](crabot-docs/superpowers/plans/2026-09-17-builtin-guidance-implementation.md)同步保留阶段与限制。
