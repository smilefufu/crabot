...
- 按用户确认的一行 diff 补齐 evidence_refs 说明：completed/partial 都只收已读末页且 gaps 为空的本周期 record_ref，无合法引用填空数组；缺口写 pending_items，Memory 结果写 summary。没有更改字段、完成门槛或退出流程；修正后的实际运行结果另核验。
