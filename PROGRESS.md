...
- 按用户确认的正文独立维护 Manager、builtin Worker、每日反思提示词；Worker 不注入 Admin 人格，保留规划、切分、执行、验收与返工闭环，模型档位仍由运行配置选择。
- 主控在判定需要人类介入前先查聊天历史和记忆，沿用有效结论；内部执行、配置和交接问题先自行诊断处理，只有明确新增缺口才向人类提问。
- 删除旧普通 Agent 提示词、装配器及已无生产入口的执行循环和专属测试；保留当前 builtin 使用的后台实体、退出通知和兼容查询服务。未修改模型配置或部署运行实例。
- 269 项定向验证通过；合入最新 main 后 Agent 类型检查与 diff 检查通过，补充回归 437 项中 423 项通过，13 项失败在远端基线复现，另 1 项重启时序失败单独复跑所在文件 26 项全部通过。全量测试有 2 个文件未结束，已中止，不计为全量通过。最终拼接正文及前后 diff 已生成供审核。
- 最终上线前必须结合工具发现的实际可见工具、加载时机与返回语义再次审核 Manager / 每日反思相关提示词。
