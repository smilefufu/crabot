...
- 9 月 21 日用户确认找不到的旧执行详情直接跳过并计为已处理。宿主保留 gaps、原引用与冻结源，持久化 skipped，排除其完成阻塞和待续读计数，但拒绝把跳过项作为已读证据；目录和其他业务门控不变。13 项失败复现后，4 文件 97 项回归及 Agent 构建通过。当前状态内存核对可处置 1,411 条已知缺失；2,687 条迁移记录和 14 条旧误报仍可正常恢复。原目录引用/顺序、详情已读标记、生产状态和水位不变；尚待部署后的真实流程验收。
