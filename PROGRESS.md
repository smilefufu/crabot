...
### 主控与执行器内置 Guidance：修复版决策比较完成，完整任务验收未完成
...
- 用户明确确认外发范围后，以修复版 `ee4c5c6d` 完成 40 条新决策轨迹、57 次请求、234,844 tokens，无重试/补样。主控旧版 8 符合/1 违反/3 证据不足，新版 12 符合；执行器两版均 4/2/2。总 tokens 少 9.5%，未缓存输入多 17.6%；新增三次请求走到派发/续办选择，贡献 3,520 未缓存 tokens。只支持有限决策观察，不能当完整任务成功率或费用结论。
...
- 既有全量测试曾中止，基线复查重现 9 项已有失败，未将其余失败全部归因；本轮只报告上述相关测试范围。后续完整任务模型比较仍需处理主控访问 Docker 内路径及部分记忆 RPC 的测量缺口，冻结新输入、版本和预算。
- 详细证据见 `crabot-agent/eval/guidance/RESULTS.md` 和[实施计划](crabot-docs/superpowers/plans/2026-09-17-builtin-guidance-implementation.md)。
