...
- 补齐内置 Skill 退役：删除仍被 Admin 注册的 `memory-graph-linking` 载荷，规则沿用已有直接注入的工作流，不再维护独立 Skill。68 项注册/快照回归通过，覆盖旧内置项清理、同名用户 Skill 保留及重启后的持久化；不改变图谱规则、权限或业务执行入口。
- 已确认两个核心与七份工作流归位：任务板概念常驻核心，自省和执行器反馈按场景加载；普通改板直接执行，管理员更新仅带短提示。保持权限、缓存、每日反思及子 Agent 隔离契约；产品正文仍为 ea1103f1。
...
- 按用户指定路线直接发布 main；同步已合并搜索修复后的 330 项定向回归、12 项 Docker 场景及 Agent 构建通过。2026-09-18 15:25 已定向重启本地 Agent，九份编译正文与确认稿一致，健康检查及启动对账完成。尚无部署后的真实自省样本，不将部署成功当作线上行为验收。等待/搜索预览及每日反思新接口另行处理；完整历史报告和未通过项均保留。
