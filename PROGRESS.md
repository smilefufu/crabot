...
> 最后更新：2026-08-09 — worker-scoped MCP capability 已经 Claude review 批准并自动合并 PR #84

## 2026-08-09 — 非 builtin worker 的 task-scoped MCP capability 接线（已合并 PR #84）

- 这是 Manager/Worker 双 Agent 架构 P3 的实现遗漏修订，不是新增设计：正式协议已更新到 `protocol-agent-v3.md` v3.0.8，原 split spec §8.2 与实施计划 `2026-08-09-worker-mcp-capability-wiring-plan.md` 已确认并推送文档仓（基础修订 `ebe076b` / `8c9f917`，review 收口至 `45364bf`）。
- harness 新增跨实现 `workers/<worker_id>/context.json`，只原子持久化 spawn 时固定的 `principal_permissions`；新 worker 无 principal 时写 `{}`，历史 ENOENT 走既有 fallback，损坏/未知字段/读写失败 fail-loud。上游旧快照缺少后来新增的已知权限类目时，写入前用 `false` / `none` 做 fail-closed 归一化，并把同一规范化快照交给 capability provider 与 adapter；spawn 在 provision 前落盘，handoff 在写交接材料或 kill 源化身前读取。目标 adapter 的无副作用 `preflightProvision` 同样在源 teardown 前运行，正式 provision 在 teardown 后重检，避免确定性 workspace 失败留下“旧的没了、新的没建成”；CLI→builtin 不再丢权限快照。
- production `capabilityBundle` 在每次 spawn/handoff 现取当前 `agentConfig.mcp_servers`，用 `BUILTIN_WORKER_PERMISSIONS ∩ principal_permissions` 收敛后按 server 过滤：`computer-use→desktop`，其余（含 Git MCP）→`mcp_skill`；CLI 物化保留 stdio `env`、远端 `headers/http_headers` 与 Claude 远端 transport `type`，避免需要凭证的 server 被静默下发成半配置。Claude spawn/resume/fork 均使用 `--mcp-config .mcp.json --strict-mcp-config`，不再与宿主 user/local MCP 求并集。两种 adapter 的 credential target 都先做 Git tracked 检查、原子替换并收紧为 `0600`：Claude 未跟踪目标/crash temp 由根 `.gitignore` 排除；Codex 先落 `.codex/.gitignore` 再写 `config.toml` / `auth.json`，已跟踪同名文件均在任何 provision 写入前 fail-loud，普通 `git add -A` 不会误提交凭据。Claude project-scope 根 `.mcp.json` 及根 `.gitignore` 副作用是 v3.0.8 明确接受的当前边界；迁移到 Crabot-owned per-worker 外部路径需另行调整寻址并验证 trust flow。skills 仍为空，manager 工具面不变。
- 验证：review-fix 定向 **6 files / 168 passed**；最终 Claude/Codex/物化安全套件 **3 files / 174 passed**；handoff preflight 回归 **42 passed**；workers/harness + manager + provision + MCP 回归 **31 files / 605 passed**（`TMPDIR=/private/tmp`、串行）；Agent 全量基线 **2623 passed / 1 failed / 2 skipped**，唯一失败仍为未改动 `tmux-driver.test.ts` 的已知 foreground-command 环境差异（期待 `sleep`、实际 `bash`，隔离重跑复现）。Agent TypeScript build 与 `git diff --check` 通过，未生成 `crabot-agent/package-lock.json`。GitHub 自动 review 指出的 MCP 连接字段、旧部分权限兼容、宿主 MCP union、两种 adapter 的凭据 mode / tracked target / ignore 顺序 / crash temp、handoff teardown 时序均已修复并补回归；MCP 逐工具黑名单和 Claude project-scope 配置路径两项边界已在协议 v3.0.8 明确，未来 schema 升级仍须先做显式 context migration。Claude 在 latest head `d14b2b1` 给出 **APPROVED，无新发现**，GitHub Actions 随后自动 squash merge PR #84 为 `3836c49017d46ff142f700c6274916db79009c54`（`github-actions[bot]` 执行），未人工 merge。
