...
### Worker 台账读取与启动后候选整理优化

- 按已确认的恢复优先 spec：保持启动恢复全量目标与现有每批 8 个，不因年龄或数量丢弃应恢复的 Worker；候选整理跳过 closed 历史，每 32 个让出执行时间，不同 Manager 串行公平排队。
- LedgerStore 按文件版本缓存已校验台账，单 Worker 查询使用索引并复制目标对象；外部修改、写入、异常及迁移均使旧缓存不可复用，正文缓存预算 64 MiB，超大文件沿原读取路径。
- 354 项相关测试通过；恢复测试 1 项旧事件 detail 精确断言失败已在未修改主分支复现。Agent 类型检查通过。100/500/1000 历史记录后的 40 个存活目标全部被探测并归 revived，未重新 spawn/kill；锁等待期间状态变化仍读取新值。
- 本机合成 1000 条台账逐项查询，首轮整账校验 1001→2 次、约 1920→26 ms；次轮 1000→0 次、约 1948→21 ms。该结果不是远端部署机 profile；频繁整账写入与超过缓存预算仍可能较慢。
- 9 月 26 日 21:25 本机 user mode 已更新至合并版本 `e0da0b44`，模块/Web/CLI/MCP 构建并重启完成。7 个模块 healthy；启动对账 revived=18、failed=0、unchanged=3618，重启前 36 个非终态 Worker 均保留且未新增化身。新 Agent PID 19815 的两次启动后 CPU 采样为 1.7%/3.6%，未见启动或候选对账失败。尚未部署到原问题 system mode 机器。
...
- 447 项定向检查及 Agent TypeScript 检查通过；worker-tools 的 1 项旧文案断言在未改 main 上同样失败。只改模型可见文案及检查，不改状态机、权限或投递逻辑；已随下述 4076a66d 部署。
- Harness/主控 child 可观测性已按确认 spec 实施，见下节。

### 主控整体执行状态与直接子任务观察

- 按 9 月 26 日确认 spec 更新正式协议；状态查询、详情和既有回合/状态事件携带同源 `execution`，保留父级状态与台账任务状态。活动 child、后台实体和持久待投递通知均可见；读取失败保留成功部分并明确 unknown，不触发恢复或新增唤醒。
- 普通主控沿用 worker 工具族按需加载直接子任务列表、详情和独立 trace；校验会话、child 和游标归属，共享既有分页、脱敏与保留副本服务，不进入每日反思受限工具面。
- 已覆盖真实 UnifiedAgent 装配、两个 child、跨化身、待投递通知、读取失败、权限与游标，以及既有生命周期守卫；10 个测试文件中 548 项通过，TypeScript 检查通过。扩大验证发现的 5 项旧失败均在未改 main 复现（旧文案断言、权限夹具与缺模型测试超时），未混入修复。复查修复自动快照调用 CLI child reader 导致事件阻塞的问题，CLI 缺少本地来源时明确 unknown，改为显式查询；builtin 按 Worker 只读取一次。
- 9 月 26 日 20:44（Asia/Shanghai）以 main `4076a66d` 完成模块、Web、CLI、MCP 构建并重启本机生产实例；新 Agent PID 98264，七模块与端点健康，Admin 资源匹配构建。部署后行为效果留待新增 trace 观察，不能用健康检查替代行为验收。

### Admin Web 输入与登录体验修复

- 发送模式切换为 Enter / Alt+Enter（Mac 为 Cmd+Enter），提示与触发一致；组合输入及上屏 Enter 不再误发消息。
- 登录后已配置全局默认模型或 Agent default slot 时进入聊天，未配置时进入模型配置；配置查询失败仍允许进入聊天，密码与临时密码保护逻辑不变。
- 新增回归先复现失败，聊天与登录 20 项测试、Web 类型检查及生产构建通过；仅模拟输入法事件，真实系统输入法待手动验收。未部署。

### 空目标提醒：后续责任与执行承诺

- 按确认文案补充阶段目标与整体要求的区别，并结合最近答复核对执行承诺、可选建议和真实阻塞；提醒触发、归档权限、自省计时和工具结构不变。
- 同步正式协议及原设计文档；工作板工具与存储 31 项定向测试通过，提醒正文逐字对齐；行为改善仍需后续真实回合验证。

### 子 Agent 异步结果与等待说明修复

- 修正传入 `agent_...` 时仍推荐未装配的 `get_subagent_output` 的错误回包，沿用异步完成通知返回结果和结束本轮等待的既有契约；不新增工具或改变通知、权限及等待语义。
- 非阻塞与阻塞两种误用均先复现失败；修复后后台工具 22 项、builtin 生产装配提示词 2 项通过；扩大运行链检查的 3 项失败在未改基线上同样复现。未部署。
- 按确认文案补齐 `delegate_task` 生命周期说明：`launched` 仅表示启动，通知直接返回结果或错误，无其他工作时结束本轮不会终止子 Agent，通知后继续父任务。系统提示词和 guidance 不变；新增说明检查先失败后通过，委派工具、后台工具及生产装配共 40 项定向检查通过。

### 微信图片按需获取高清版本：Crabot 已部署，端到端待验收

- 按 9 月 25 日确认的 [spec](crabot-docs/superpowers/specs/2026-09-25-wechat-hd-image-on-demand-design.md)，微信图片默认只提供存在性、质量和取图引用，避免缩略图进入视觉输入；主控按需获取高清，或仅取得文件路径交付 Worker，权限不变。
- 系统在原取图调用内等待最多 120 秒，重复请求合并；取消、新输入和超时结束等待，迟到结果不另行唤醒。质量未知、尚未就绪和下载失败分别反馈，主动读取保留高清字节。
- 覆盖群聊/私聊历史、引用消息、高清升级缓存、实际工具装配与取消等回归。Connector 按既有协议修复二次上报被去重丢弃，独立提交。9 月 25 日 19:10 以已合并 main `df351fde` 完整构建模块/Web/CLI/MCP 并重启本机 Crabot；七个模块 health RPC 全部 healthy，微信 Socket.IO 已连接，线上 capability 确认支持 fetch_image，Admin 资源与构建一致。本次未部署 connector，真实高清补报仍待两端就绪后验收。

### 主控/执行器行为优化实验：候选已完成真实 A/B，已提交并更新本地部署
...
- 新增 15 项回归，相关 8 个测试文件共 114 项通过；Agent 类型检查和构建通过，验证真实失败、后台退出码/通知、输出读取和整组子进程取消。仅修既有执行契约，不改提示词、权限、存储或状态机；未重启生产实例，不据此宣称全系统无 bug。

### 每日反思宿主管理分页：已部署，原周期验收通过
...
- 分页及其余定向回归共 311 项通过；两项 Memory trace 旧断言在干净基线同样失败。review 的目录重发预算变化已补回归并局部修复，84 项和构建通过；生产状态副本保留全部 15,010 条引用/来源和既有详情处置。
- 9 月 22 日 22:14 正常部署一次并恢复原 episode，模块健康、编译产物核验一致；真实无游标读取跨多次压缩继续到 EOF。9 月 23 日 00:24:59 原周期 completed，目录 15010/15010、23 条已选详情完整、校验无错误、水位到达冻结终点；Memory/links/摘要及保护边界对账通过，未额外触发 Schedule。
...
- 9 月 22 日 16:04 随已合并会话执行状态版本完成构建和本机部署；实际静态资源与构建产物逐字节一致。

### 会话整体执行状态：已部署，运行接口已验证
...
- 自动审查修复 legacy 已停止记录误报未知、后台退出通知误报运行；269 项定向回归及 Agent 类型检查通过。9 月 22 日 16:04 同步远端 main `c8983b2a`，以 Node 22 完整构建模块/Web/CLI/MCP 后执行一次 `crabot restart`；新 Agent PID 32237、启动对账和真实列表三态字段已核实。Telegram 首次启动遇上游 TLS ECONNRESET，16:07 单独重启恢复；MM/Admin 端点及全部七个模块健康，页面 JS/CSS 与构建逐字节一致。未主动触发业务任务。
...
- Agent 核心 361 项、Git 18 项、CLI/tmux 停止 17 项及 list_workers 定向测试通过；最后通知保护修改后，候选/生命周期 170 项及 Agent 类型检查通过。Web 49 项通过，子 Agent 详情断言在干净基线同样失败；协议与实施计划同步。
...
- 9 月 22 日 10:25 完成 main 同步、完整模块/Web/CLI/MCP 构建及 `crabot restart`；MM、Admin、Agent、Memory 和全部渠道健康，新页面资源与构建哈希一致。启动对账成功，首次核验关闭 224 个闲置执行器；默认视图保留 6 个续办候选、33 个待核实条目，未强行清理未知状态。启动仍有既存 Scrapling 缺失与历史 trace 恢复警告，不宣称历史异常已全部消除。

### 上下文容量恢复：已部署，真实压缩后续跑已验证
...
- 边界回归先复现 9 项失败，核心 372 项通过；review 补齐持续故障期间新输入保留、跨模式与并发恢复八种组合，最后相关 179 项通过。全量旧失败与时序差异已对照主线，未声称全量绿。
- 9 月 22 日 00:11 完成构建与 Agent 重启，原 run、冻结窗口与每日反思状态跨重启完全一致。此后 8 次真实压缩成功并继续原 episode；首次主请求总输入含缓存从 326142 降至 274408 token，后续也持续低于 320000 阈值。极端分段/裁剪分支仍以定向测试为证，不声称生产触发。每日反思最终主动提交 partial，业务完整验收未通过。

### builtin Worker 请求边界投递与运行观测：已部署，线上观测验收通过

- Worker `w-87a2bcf7` 的主模型调用内部尝试 6 次、历时 5463 秒后以 502 退出，3 条已接受输入始终留在队列；外层 turn 消费没有覆盖内部请求重试边界。
...
- Engine 660 项、适配器 66 项、Worker/Harness/trace 定向回归及 Web 23 项通过；review 修复防止运行诊断覆盖 Manager 最近活动和反思错误摘要，相关 215 项通过。扩展测试的旧重试假时钟及受限进程时间测试问题已对照基线复现，未宣称全量绿。
- 9 月 21 日完成主仓同步、全部模块及 CLI/MCP/Web 构建和实例重启；7 个模块健康，线上 Admin 静态资源与新构建一致。真实 Worker 只读接口已返回主请求阶段、模型、尝试次数、开始时间及待注入数；未人工重放事故任务或构造生产投递测试。
- 启动时 Admin 注册发生一次 `ECONNRESET`，通过 MM 内置模块重启恢复；注册失败根因尚未确认，作为独立 follow-up，不混入本次投递修复。
...
### 每日反思取证与完成闭环：原周期真实验收通过，自动计划仍暂停

- 9 月 23 日 00:24:59（上海），原 run `1021e970`、episode `8f8c90e1` 正式 completed；宿主 validation_errors=[]、confirmation_pending=false，水位由原起点推进到冻结窗口终点 `2026-09-18T18:00:00.006Z`。目录 15010/15010，23 条已选详情完整，无待续详情或未处置缺口。
- 相对本次单次续办前基线，新增 5 条 confirmed 记忆、5 条关系；29 条旧记忆中 28 条仅更新使用/核验统计，1 条另更新 brief/body。无记忆或关系删除，5377 条受保护历史 inbox 逐字段不变，正常 inbox 为空。摘要正文与消息 ID `5a49553c-4b8d-457d-bb2d-483ae59e52ef`、时间在 Admin 实际落库中匹配。
- 原 run、冻结窗口及全部来源引用/顺序保留；1420 条找不到的旧详情按用户授权 skipped，保留缺口和来源，不当作证据引用。每日反思由 Manager 执行，未为取证新增 Worker。Schedule 仍关闭、execution_count=75，验收期间未额外触发。
- 已修复 Harness 历史投影/hash 与 legacy trace 归属误诊断、正常 inbox 筛选及历史 inbox 写保护、提交前引用校验、可推进证据退出守卫、宿主管理分页。13:10 提前 partial 被守卫拒绝后同 episode 继续；无游标实现部署后跨真实压缩继续读到 EOF，无新增工具错误。Provider 502/idle timeout 经既有恢复继续，不计为分页故障。
- 分页实现与定向测试见本文顶部；原 spec、正式协议、实施计划已同步。历史过程和最终对账见 `/private/tmp/crabot-daily-exit-acceptance-20260922/REPORT.md`、`detailed-audit.json`、`final-verification.json`；上轮 partial 及旧失败保留在报告，不再作为当前状态。
