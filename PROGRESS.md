...
### 主控工具搜索边界：已修复，待 PR 与运行验收

- 按 9 月 18 日已确认方案先固定相关结果集合，再区分新增与已可见工具；重复查询不再用长尾补位。精确名称/alias 仅返回目标，不存在的规范名称返回 no_match。
- 7 项回归先复现旧实现失败，目录、工具装配、执行条件和 MCP 授权共 93 项通过，Agent 构建通过；既有 48 项工具的 96 条中英文查询与精确名称全部命中。每日反思 9 条真实查询重复四次的新增量从每条 12 降至 3。
- 宽自然语言仍有弱相关误返回，不能宣称整体搜索相关性达标；未部署。每日反思取证/完成闭环独立推进，提示词与 guidance 由“梳理主控与执行器提示词”任务处理。
