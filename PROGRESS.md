...
### 运行记录执行器统计布局与待核实说明：已验证，待部署

- 会话统计改为两列对齐，零值弱化，执行中与待核实区分颜色；任务板短语整体换行。会话列表、详情和执行器汇总统一使用“待核实”，不再用“异常”混淆未知证据与任务失败。不改变分类、计数或停止行为。
- 只读核实 bot-2 会话 9 条待核实：5 条 builtin 元数据缺失，3 条停止后台资源未确认，1 条旧任务 running 与已退出主线不一致。当前 attention 分类本身不保证逐条唤醒和处置；具体原因展示与系统核验、Manager 业务处置闭环作为独立设计 follow-up，不强行清理旧记录。
- 相关 42 项测试、ES2022 类型检查和 1440/1024/768/390px 浏览器检查通过；页签切换正常，无统计断行或页面横向溢出。默认 tsc 仍有既存 Array.at 编译库错误；本次未调整构建配置或重启 Agent。
