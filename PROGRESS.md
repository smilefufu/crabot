...
### 每日反思宿主管理分页：已验证，待 PR 审查与部署

- 按原每日反思 spec 的 9 月 22 日确认修订，模型只提供 record_ref/restart，用 has_more 继续；目录与各详情分别保存位置，EOF 默认不重开。提示词仅应用已确认 diff。
- 待确认页先持久化，再由 Manager 已保存的真实工具回包确认推进；恢复缺回包重发原页，重复调用不跳页，末页确认前不计完整。页状态或检查点保存失败在下一次推理前结束本轮，后续合法回合从原持久位置重试。
- 分页 14 项专项覆盖及其余定向回归共 311 项通过；两项 Memory trace 旧断言失败在干净 c8983b2a 基线同样复现。Agent 类型构建通过。生产状态副本验证 15,010 条引用/来源、33 条详情处置及原窗口保持，未改生产数据。
- 未部署；后续按既有授权正常部署一次并恢复原检查点，核对真实读取、Memory/links/摘要和水位。测试和 PR 不等于完整业务验收；不额外触发 Schedule。

### Worker 历史活动展示收敛：已验证，待 PR 审查

- 按 9 月 22 日确认的展示规则，最新进度只在“当前执行”更新；默认历史隐藏正常请求、重试和输入过程提示，保留实际输入正文、回复、工具及错误。历史接受回执转入技术事件，输入正文标为“输入”；压缩失败和恢复前错误仍可见，不改后端或存储。
- 两项回归先复现，修改后 Worker 页面 24 项测试、默认 Web 类型检查和 Vite 生产构建通过；Playwright 合成数据验证 1440px/390px 下最新快照更新、工具展开保持、技术事件切换，无页面溢出或控制台错误。未重启或部署生产实例。

### 会话整体执行状态：已验证，待 PR 审查

- 按 9 月 22 日确认的 [spec](crabot-docs/superpowers/specs/2026-09-22-manager-execution-status-design.md)，列表用“执行中 / 当前无执行 / 暂不可用”替换四项执行器内部计数。主控真实回合或所属 Worker 任一执行即显示执行中；驻留实例、旧 task running、候选资格与待投递通知不替代运行事实。协议及计划已同步文档仓 main。
- 只读聚合复用现有启动探测、状态回调及所属执行观测，不重建 runtime 或修改台账。启动未就绪和观测失败保留未知；补测复现并修复旧台账覆盖启动 idle、旧 running 回调覆盖已核实停止两类误报。页面可见时每 5 秒串行刷新，失败使状态失效，翻页及卸载丢弃旧响应。
- 相关 Agent 418 项、Web 28 项通过，Agent/Web 类型检查与 Web 构建通过；另有两项每日反思测试失败已在原 main 同样复现。1440/1024/768/390px 浏览器验证状态、刷新失败及恢复、导航和布局通过。未部署、未重启生产。
- 合入同期 main 后仅进度文档冲突，保留双方记录。扩大验证：CLI 193 项通过，Web 309 项通过、12 项 localStorage 失败在原 main 复现；Agent 全量存在既有断言、受限端口/tmux 及超时问题，重试用例持续阻塞后中断，不能宣称全量通过。范围外失败未混入本次修复。
- 详情和排查页保留“待核实”；现有 attention 分类不保证逐条唤醒和处置。具体原因展示与系统核验、Manager 业务处置闭环仍为独立 follow-up，不清理旧记录。

> 最后整理：2026-09-16
> 本文件只保留当前状态、明确 follow-up 和阶段性里程碑；详细实施流水、逐轮 review 与历史测试输出见 Git 历史。压缩前完整版本可用 `git show 49b9cb4:PROGRESS.md` 查看。

### Harness 启动对账与 Manager 恢复循环等待：已验证，待部署

- 现场回合 `e19ba425` 重启后停在恢复闸门：启动对账等待旧 Worker 通知被 Manager 消费，而 Manager 续跑又等待对账结束。按既有恢复协议移除完成/activity、input、query、control 通知消费对启动对账的阻塞；状态与回执仍先落盘，通知保持原确认和失败重试规则，无路由的 control 审计仍同步写完。
- 四类真实装配回归均先复现超时，修复后验证对账收口、原检查点恢复、通知不提前确认及后续人类输入；Harness 生命周期、恢复与 Manager 装配共 254 项通过，Agent 类型检查通过。未重启生产实例。
- 后续独立处理：管理回合页面区分实际执行与等待恢复，显示最后活动及阻塞原因；移除固定“刚刚更新”造成的进展误导。

### Manager 运行记录追加消息缺失：实现完成，待 PR 审查

- 按 9 月 20 日确认 spec 保存回合初始批次与 mailbox 已注入的人类消息摘要；排队消息不归入旧回合，trigger 保留原始启动原因。恢复按原回合与消息 ID 合并，观测失败不重试业务调用。
- 详情页按消息平台时间保留完整回合输入，已结束不再退回只显示“继续”；最近成功发送回复与各操作分别带时间，跨接口轮询交接保留已观察消息并明确缺口。
- 截图回合 `66402202` 的三条输入已逐项核实，候选离线投影显示 22:05 最新消息和 22:10 最近回复；定点补齐不重放业务。Agent 202 项、Web 26 项定向测试通过；桌面和 390px 窄屏预览已核对。Agent 与 Web 构建通过；尚未部署，不自行合并 PR。

- PR #180 review 缺口修复：普通失败与 throw 归档前检查未 drain 插话，标记 partial；TraceStore 允许降级并保留已知缺口。消息归属及重投不变，相关 186 项回归通过。
...
### 执行器续办候选与资源淘汰：实现已验证，待 PR 审查

- 按已确认 [spec](crabot-docs/superpowers/specs/2026-09-21-worker-continuation-candidate-limit-design.md)，当前目标涉及项目去重后，每项目最多 3 个闲置续办候选；淘汰核验并关闭所属资源，分支、子 Agent、后台执行及待投递通知保护执行中状态。
- Manager/Admin 共用只读候选集合，新增运行、排队、候选与异常计数；保留全部非终态兼容计数及历史分页，不增加 objective_id 或 Manager 搜索。启动对账后启用自动淘汰，失败或归属不明保留异常、不盲目重试。
- Agent 核心 361 项、Git 18 项、CLI/tmux 停止 17 项及 list_workers 定向测试通过；最后通知保护修改后，候选/生命周期 170 项及 Agent 类型检查通过。Web 49 项通过，子 Agent 详情断言在干净基线同样失败；ES2022 lib 下 Web 类型检查通过，常规检查保留既有 `.at()` lib 错误。协议与实施计划同步，未部署或清理线上资源。
- PR #185 首轮 review 两项真实风险已复现修复：只读列表不再调用可重建 CLI runtime 的原生入口；项目身份每轮复用，淘汰仅复核目标及同项目最多 3 个保留者，任务板不再全程锁住。80 Worker 核验次数上界、改板及重新取得名额等 349 项回归和 Agent 类型检查通过；巡检钩子保留，未改变配额、归因或持久化契约。
- 只读列表复用自主对账的项目身份快照，范围变化或新 workspace 未观测时保留待核实；实际淘汰仍使用新鲜核验。重复轮询零新增 Git 解析等 230 项通过；合入同期 main 后定向 234 项通过、5 项 tmux 跳过，Agent 与 ES2022 Web 类型检查通过。全量未全绿：Web 12 项 localStorage 失败在干净 main 复现；Agent 存在基线失败、tmux 环境波动，两边均因既有重试假时钟用例阻塞而中断。
- 9 月 22 日按用户确认的停止边界修复三条 review：已退出 CLI 的历史子 Agent 不再阻塞候选或停止核验，builtin 独立子 Agent 仍按实时事实保护。主线总结和结束时间独立持久化，聚合排序取所属执行最后真实停止时间；重启、通知延迟及后台记录 GC 不刷新时间，证据缺失保持待核实。新增 15 项生命周期及 1 项后台归属回归，相关 280 项测试、Agent 类型检查通过。
- 启动阻塞回归先复现后修复：既有对账及 recovered bg 释放之后，后台启动候选淘汰，不等待持续重排的 sweep 收敛；恢复通知和活性巡检正常启用，失败沿原 catch 记录。224 项定向回归及 Agent 类型检查通过。上线观察系统淘汰通知量；工具缺读板的测试装配及观测缓存回收另作 follow-up，不改当前通知契约。
...
- 移除固定绿色“刚刚更新”，分别显示回合数据刷新时间与刷新失败；当前执行器和会话计数跟随轮询。技术详情按实际状态展示，展开及窄屏布局保持可读。
- 30 项页面回归、Web 类型检查和构建通过；Playwright 合成数据验证 1440px 与 390px 下的展示、详情展开、标签切换及刷新失败提示，无横向溢出，正常状态无控制台错误。
- 页面局部显示修复，沿用现有只读 API，不改 Agent 状态、通知或恢复语义；按用户授权验证后直接合入 main，部署状态单独确认。

### Manager 答复投递与收尾修复：限定 Kimi 完整循环验收通过

- Manager 的消息工具说明移除不存在的 intent/ask_human/waiting_human 和旧 loop 语义；短核心补回答复投递、结束及不重复发送责任，保持 299 字和现有 15 项工具面。不改 adapter、共享 handler 或结束状态机。
- 复现先失败；四文件 213 项与 Agent 构建通过。真实 Manager loop 接 Kimi k3 共 6 次请求：两条正常问答各发送一次后结束；重复拒绝、模拟空内容拒绝后均立即结束，没有改写重发。首版未投递失败记录保留，见 `crabot-agent/eval/manager-send-message/RESULTS.md`。
- 仅证明短合成场景；生产空内容校验及原事故长历史不在本批验证内。未部署。
...
- 覆盖历史、运行中多图插话、Worker 事件唤醒和重启恢复；最新图失败不回退旧图，远程读取沿用 8 秒媒体上限，连接重试复用选图结果；私聊及工具截图规则不变。
- 按用户确认统一修复非人类唤醒身份：群主控绑定完整渠道/会话并持久化，旧群从精确 Channel 查询恢复；私聊沿用可信 Friend 绑定，群发言者不再作为权限/Memory 主体。Worker 通知、任务板及自唤醒的项目授权统一取主控权限，调度管理保留有效私聊 Master 授权，撤权/查询失败不得借用默认执行能力或 public Memory 范围。
- 群绑定恢复和 Worker 来源权限错用先失败后通过；真实 bootstrap 覆盖 Worker、任务板、媒体通知、非人类检查点恢复及旧群升级，另覆盖私聊 Master 降权、权限/联系人查询失败。定向 429 项通过、Agent 构建通过；扩大 Manager 检查的工具文案断言失败已在 main 同样复现，未改无关提示词。完整 diff 已复核，未调用真实模型或私人图片 URL，未部署。
- 新增 group binding 不能直接由旧二进制读取；旧群无绑定且 Channel 查询失败时拒绝猜身份并保留现有检查点。上线后仍需观察真实超时改善。

### 任务板空目标复核：实现与定向验证完成，待 PR 审查

- 最后一项完成、放弃或移走后，当次工具结果提醒 Manager 核实源目标；Admin 更新通知及自省指南补充空目标判断，页面说明事项为空不代表目标达成。
- Store 在同一互斥修改中识别非空到空转换；不自动归档目标、不强制建项或沟通，不新增计时器、持久化状态或权限。
- 任务板、提示装配、通知/恢复/自省及页面共 287 项通过，Agent 类型检查通过；尚未做真实模型行为验收或部署。

### 主控工具按族加载：实现及限定模型对照完成，待 PR 审查

- 按用户 9 月 19 日确认修订原工具精简 spec：memory/messaging/worker/schedule/crabot 按族完整加载；外部 MCP 可列服务、按服务获取或搜索。普通核心 15 项，daily 11 项仅替换发现入口，graph 3 项不变。
- 保留 profile/权限/调用守卫及 episode 追加与重置；内置工具不再进入搜索索引，检索异常不展开全量内置工具。核心/guidance 经责任任务核对无需修改。
- 191 项定向测试及 Agent 构建通过；9 个离线场景明确显示同族多步可少发现调用，单工具/跨族可能多带 schema。首轮 +759 bytes；不是 Provider token 节省证明。见 `crabot-agent/eval/tool-families/RESULTS.md`。
- 用户授权后完成 7 场景/14 轨迹、48 次真实模型请求，无 Provider 错误。记忆三项操作 6→3 轮、输入 -41.1%；4 个可比场景合计 14→12 轮、总 token -9.8%。单项/外部族不保证节省，外部服务先列目录反而多一轮；两条旧版未完成和一对夹具非法 ID 已排除收益统计。夹具续接、参数审计及局限见报告；业务 handler 未执行，未部署、不自行 merge。

### 主控每轮自适应压缩：原始输入保护范围修复待审查

- 9 月 21 日修复容量判断提前收口：同一主请求的完整估算与含缓存真实用量配对，压缩后保留已测低估偏差，继续处理仍超预算的大消息；模型或 adapter 切换重置，未改提示词、阈值、原文保护和持久化契约。Manager 20 条与 builtin 6 条回归先失败后通过，相关 385 项测试及 Agent 构建通过；扩大检查中的四项既有失败在未改 main 同样复现。已按小改动流程发布并部署，原每日反思 episode 从检查点续跑，重启前后 dailyReflection 状态完全一致。模型侧压缩收益和反思全周期仍待终态验收。


- 按 9 月 17 日已确认设计接入 Engine 每轮容量检查，主控只装配输入保护、摘要和批次持久化；超限恢复保留已执行工具结果，不回到 episode 起点重跑。
- 补齐百炼 SSE 永久错误分类与一次有限压缩恢复；新增 usage 越阈值、超限、重复超限、压缩后失败与插话不丢不重回归。定向集合 268 项通过；1 项旧提示词断言在基线复现，后续新增二次超限用例通过；Agent 构建通过。重启测试旧摘要识别夹具同步当前 profile 后 18 项通过。
- 适配器、builtin 执行器和主控集成另 158 项全部通过。本机已部署候选 Agent 构建并正常重启，MM／Admin／Agent 及所有渠道运行，产物校验一致；源码合并仍按 PR 审查流程进行。
- Review 修复失败事件重复入参及复核续跑失败覆盖成功压缩批次；原 envelope 保留待处理责任，下一轮按已保存消息 ID 复用原文，同文新事件仍正常进入。三项复现先失败后通过，定向集合 223 项通过、1 项既有提示词断言失败；Agent 构建通过。
- 9 月 19 日按原 spec 修复过度保护：只保留当前输入原文，输入之间和之后已完成的工具组可由共享 Engine 原位摘要，旧小片段无收益时继续检查后续片段；保存与重启保留输入身份和工具执行进度，不扩大为整轮不可压缩。
- 两项无旧历史的 92 次工具调用回归先失败后通过，涵盖人类请求、定时触发与途中插话；片段滚动摘要、失败部分保存和重启续跑等定向集合 380 项通过，Agent 构建通过。昨晚 207 条真实消息的本地只读投影中，旧边界选不到本轮工具结果，候选选中 82/92 组；使用固定替身摘要，不证明模型摘要质量或反思业务完成。候选未部署，不修改线上水位或冻结目录。

### 每日反思取证与完成闭环：真实验收未通过，自动触发已暂停

- 9 月 22 日已确认并实现可推进取证退出守卫：目录未完且无实际目录故障、或已选详情未完且无缺口时，completed/partial 都在原 Engine 内拒绝退出。目录故障只记当前宿主及 run，成功后清除；不增加持久字段、自动翻页或 Worker。详情提示词仅改为优先已有 next_cursor。相关 274 项定向测试及 Agent 类型检查通过；两项旧 20 条分页断言已在未修改主线复现并排除，未宣称全量通过。待 PR 自动审查、部署及一次原周期续办验收。

- 最新现场：用户明确允许后已单次触发 episode `b53051f9`；正常 inbox 候选经比对后确认并新增一条 refines 关系，5,377 条历史 inbox 未变，八条旧误诊断已解除。原 run、冻结窗口、来源和水位保持，execution_count=74，自动计划仍关闭。长详情处理中暴露共享 Engine 压缩提前收口，已修复部署并从同一 episode 检查点恢复；目录仍未读完，尚未完成摘要投递与完整验收。


- 9 月 21 日用户确认续读与正常候选查询修复：目录改为最多 100 条/80 KiB 完整回包，持久保存最后一页实际范围并兼容旧 20 条游标；进度返回最多 20 个未跳过缺口引用，解决已翻过却从未读取详情的记录无法定位。daily inbox 由宿主强制正常候选筛选，Memory 在 SQL 分页前排除缺少 inbox_entered_at 的历史项，并拒绝 daily 错套执行窗口；历史写入守卫保持。三处已确认 skipped 提示词同步。复现先有 Agent 12 项、Memory 5 项失败；修复后相关 Agent 184 项、Memory 生命周期集合 304 项通过，Agent 构建通过；registry 两项既有失败在未修改主仓同样复现。真实冻结目录只读投影返回全部八条缺口引用，从原 2160 起点续读 129 页，最大完整 JSON 81221 字节，生产进度不变。修复已自动审查合并并部署，Agent 构建、Memory/Agent 重启及健康检查通过；生产只读筛选准确返回唯一正常候选。实际触发被自动审批拒绝，尚无新 episode；执行次数仍为 73，Memory/links、历史 inbox、原 run/窗口/冻结来源及水位不变，真实模型验收仍待执行。
- 9 月 21 日用户确认找不到的旧执行详情直接跳过并计为已处理。宿主保留 gaps、原引用与冻结源，持久化 skipped，排除其完成阻塞和待续读计数，但拒绝把跳过项作为已读证据；显式读取仍分页返回尚存正文，目录和其他业务门控不变。13 项失败复现后，审查修复共 4 文件 99 项回归及 Agent 构建通过，已部署。
- 9 月 21 日 15:49 至 16:08 当前实例实测（episode `db60b3f0`）运行 19 分 30 秒：12 次主控请求、21 次工具调用、0 工具错误，全程 11 项 daily 核心；1,411 条旧详情缺失通过正常 list 跳过，6 条旧失败详情通过正常 read 读完。目录只从 2080 推到 2180/15010，8 条可恢复旧误诊断未复读；主控第 12 轮主动提交 partial，重复旧游标且给 inbox 错套执行窗口，漏查本日正常候选。最终仍有 directory_not_fully_read、known_evidence_gaps，整体验收未通过。Memory 全表双向比对无新增、修改或删除，links、5,377 条历史 inbox、原 run/窗口/冻结来源及水位均不变，自动触发继续暂停。剩余目录按固定 20 条分页至少需 642 次串行查询，超过单 episode 默认 200 轮；分页执行成本、正常 inbox 覆盖和提前收口仍需解决。
- legacy 详情诊断按当前 Worker 引用的 trace ID 归属；可识别为其他 ID 的无关联记录或冲突不再污染该 Worker，无引用时不扫描全库。无法识别归属的损坏、文件读取失败与选定来源缺失仍报缺口。旧冻结全库警告仅在原上界重读成功后解除，已有 digest 必须匹配，首次失败未产生 digest 的沿用既有成功重读建 hash 逻辑。4 项失败复现后相关 107 项回归及构建通过；当前实例 8 条原记录在内存会话核验均解除误报，6 条多页详情仍未标为读完，生产状态与冻结来源不变。未触发模型或改变完成条件，其他来源缺口继续核查。
- 9 月 21 日按用户要求先修第二、三步的 Harness 取证：旧冻结 worker 详情仅在原上界、现有脱敏后完整匹配原 digest 时恢复含迁移审计的原投影；纯迁移项的列表摘要明确其不是本周期业务执行证据，保留原引用、目录顺序和分页，不自动标记详情已读。新目录继续排除纯迁移任务。回归先复现 4 个失败，修复后 63 项定向验证及 Agent 构建通过。当前实例 2,687 条真实记录只读对比从全部哈希不匹配变为全部匹配，6 条失败详情恢复，15,010 条原目录引用/顺序不变，生产状态文件哈希不变；真实 Worker 输入重新取目录排除全部 2,687 条纯迁移项。这是 Harness 与真实源数据核验，不是模型全周期验收；未改提示词、搜索反馈、退出契约、Memory 或水位，真实来源缺失和其他取证缺口仍待逐类核验。
- 9 月 21 日 08:33 当前实例实测（episode `4bc2786d`）运行 23 分 47 秒：9 次主控请求、1 次历史压缩、29 次工具调用；核心 11 项，一次加载完整 Memory 族后共 24 项。目录从 1820 续读至 1880/15010，读取 4 条 confirmed、3 条 inbox 正文与 2 条有缺口的执行记录；新增 2 条已确认记忆关联，Admin 摘要真实投递。5377 条历史 inbox、记忆正文、生命周期与水位不变，自动计划继续暂停。收口仍混入 7 个 Memory ID、2 个未读引用和 2 个有缺口引用，宿主记录 unread_evidence_reference 并保留 partial，不能算整体验收通过。
- 按用户确认的一行 diff 补齐 evidence_refs 说明：completed/partial 都只收已读末页且 gaps 为空的本周期 record_ref，无合法引用填空数组；缺口写 pending_items，Memory 结果写 summary。没有更改字段、完成门槛或退出流程；107 项定向回归与 Agent 构建通过，已部署。
- 修正后 09:48 再实际触发一次（episode `0959d2f7`）：3 分 30 秒、4 次推理、10 次工具调用，目录 1880→1900；Memory 全表和关联无变化，没有重复发摘要，计划仍暂停、水位不变。仍提交 2 条有缺口引用和 1 条未读引用，校验未通过。根因是原 spec 将引用校验安排在 Engine 退出后，错误提交无法在同 episode 纠正。
- 原 spec §7.2 修订已确认并同步协议：daily 退出前只读校验 schema、单独调用及完整已读的本周期引用；拒绝整批并返回错误，模型在原轮次预算内纠正。合法 partial 不受全局缺口阻挡，退出后最终校验保留。新增回归先失败后通过，Engine/宿主/Manager trace 等 361 项及 Agent 构建通过；扩展 registry 检查 77 项通过、2 项 Memory 错误夹具在未修改 main 上同样失败，独立记录不扩大修复。该机制已部署，实际样本的覆盖边界见下条。
- 9 月 21 日 11:04 当前实例单次验收（episode `4ce1bdf8`）耗时 28 分 15 秒，13 次模型轮次、24 次工具调用，另有一次请求失败后重试成功；全程核心 11 项，主控输入含缓存 1,922,639 token、输出 41,362。目录 1900→2080/15010，5 条新增详情均有冻结证据缺口；一次合法 `partial + evidence_refs: []` 正常退出，无非法引用或提交循环，但没有实际覆盖拒绝后纠正。Memory 正文、状态、links 不变，5 条 lesson 的访问统计按搜索既有逻辑更新；未发摘要，原窗口、水位及自动计划暂停状态不变。历史覆盖、正常新候选处理与整体业务验收仍未通过。
- 本次两次把已知 Memory ID 当普通 query 搜索得到空，随后改用内容关键词已取得同两条正文及 links；当前实例精确详情 API 也均成功。搜索协议没有 ID 精确匹配语义，不能认定为后端按 ID 读取失败，也不能仅归咎提示词；待明确 Harness 对 ID 查询的处理及接口反馈，现有说明修正草稿暂缓。inbox 查询仍曾错误套用执行窗口，后续不带窗口只抽查历史候选，不能当作全量队列核验。
- 9 月 20 日 21:44 手动触发一次实际验收（episode `f99536a7`）：7 次主控模型请求、35 次工具调用，核心持续为 11 项；正确取得 1800 的保守续读位置，inbox 按最早入库排序且未套执行窗口。但新唤醒已带历史 inbox 禁止自动重判的规则，仍确认 6 条、移入 trash 14 条缺少 `inbox_entered_at` 的历史候选；未读取目录详情或记忆正文，未加载扩展工具族，未提交收口。目录仍为 1820/15010，水位未推进，不能算完整验收通过。
- 已停止错误运行并保留隔离检查点，暂停每日反思计划；20 条误处理记忆经备份、离线恢复演练及逐项元数据/正文比对后，全部恢复原 inbox，未重置生命周期时间，其余记忆不变。模块停止期间遇到 MM 的 `kill EPERM` 清理错误，经标准实例重启后所有核心模块与渠道恢复正常。
- 历史 inbox 保护已部署：可信每日反思每次确认、删除、更新或建链前核实目标当前状态，缺少 `inbox_entered_at` 的 inbox 拒绝修改，查询失败同样不写；普通主控、Admin 和正常新候选行为不变。不增加完成门槛或失败账本。回归先失败后通过；同步 main 后 Agent 163 项定向测试与构建通过。原 6 次确认和 14 次删除的 Engine 重放全部保留失败回包，20 条真实记录的只读代入也全部拒绝、零写入。
- 9 月 21 日经批准完成单个 Kimi k3 合成场景及原额度内续办：累计 16 次请求、含缓存 116,015 token、约 95 秒，未调用生产业务 RPC。模型按需加载 memory 族、读取执行详情与候选正文、确认新候选并建链、取得模拟摘要回执；历史候选不变，缺口保持 partial、水位不推进。严格验收仍未通过：两次都把末页仍有 gaps 的记录放入 evidence_refs，宿主拒绝此引用；当时“读完”的说明遗漏“末页且 gaps 为空”；本轮已结合后续真实验收证据确认说明修正。
- 合成夹具的摘要端口路由错误造成首段两次模拟发送失败，已修正并用原动作离线重放及实际续办验证，不能算生产投递故障。初版验收未检查引用与摘要回执，错误给出 passed=true；已按严格检查纠正。生产反思状态、5377 条历史 inbox 和自动计划暂停状态均未改变。本场景不是完整 ManagerLoop、真实历史全量或当前生产 gpt-6-astra 的验收；无旧版对照，不据此宣称节省 token。
- 9 月 20 日验收时，223 条旧消息使单次输入约 32–35 万 token；当时 Kimi 窗口为 1,048,576，原 spec 的 80% 压缩阈值为 838,860，未触发不构成压缩实现失效，也不能据此宣称总体 token 节省已验收。未改阈值、清空历史或重置水位。
- 9 月 20 日按用户确认 diff 落地每日反思正文、Memory 工具说明、memory 族说明与 Admin 唤醒说明：固定周期续办，局部缺口不阻止独立工作，inbox 最早优先且不套执行窗口，已知 ID 直接读详情，队列出队后重核偏移，无新成果不构成阻塞。提示词与流程修复统一由本任务负责。
- 目录进度从既有持久状态返回；补齐宿主保存进度后、Engine 保存回包前的崩溃窗口：续办保守重读最后一页，末页也保留入口，之后沿正常 next_cursor 前进。未完成详情从首段重读，不把旧游标当成无缺口覆盖。真实状态只读副本验证 1820→重读 1800→1820→1840，冻结目录及生产文件不变。
- Harness 既有 spawn_failed 事件标明 pre_spawn/spawn。只有明确启动前失败、且无同化身执行事实时，反思以冻结 Harness 失败事件取证；失败原因仍进入目录与详情。旧事件、进入 spawn 后失败及真正缺失 trace 继续报缺口，不回填旧冻结目录。
- 12 处回归先失败后通过；Agent 328 项、Admin 10 项相关测试及两模块构建通过，覆盖真实 Harness 落盘到反思目录、ManagerLoop/Engine 续办到完成或 partial。实际装配仍为核心 11 项、按族加载 Memory 18 项，确认稿内容一致；离线验证没有 Provider/RPC 调用，不证明真实模型业务完成。
- 历史缺失 trace 与缺少生命周期字段的 inbox 分别保留原取证/人工迁移边界，不自动跳过旧窗口、不批量重判或清理记忆。
- 9 月 20 日真实反思运行 6 轮、约 137 秒，11 项 progressive 核心已生效；只重读目录首页，实际晋升 2 条记忆、丢弃 4 条后提交 partial，目录进度与水位未推进。当前历史 inbox 还有 5377 条，不能将该页剩余 14 条当成全库积压。
- 已补齐 Memory 后端已有的 `list_entries.sort` 工具参数，并明确 `finish_daily_reflection.evidence_refs` 只收已读 `record_ref`；不把工具契约修复等同于每日反思整体验收。
- 9 月 19 日实际运行仅翻到 1820/15010 条目录即压缩失败，未提交完成、水位未推进；约 47 天积压与历史缺口仍待处理，不能按完整业务验收通过。
- 已修正迁移审计事件及同值 `updated_at` 被误算为执行活动的问题，同时覆盖合并 trace 中的迁移事件。新增 7 项回归均先失败后通过，相关 37 项回归与 Agent 构建通过；昨晚 2695 条迁移记录的本地只读对比排除 2687 条纯迁移旧记录，保留 8 条确实在周期内结束的任务及其证据缺口，也保留真实执行、输入和后续更新。压缩根因及已冻结目录的处置独立跟进，不改现有水位或运行快照。
- 按已确认 spec 增加 daily 目录/详情/显式完成工具，核心 8→11；固定周期跨 episode 续办，普通消息不获得跨会话读取。
- Admin 单次触发凭据核销；完成结果先持久化、再经认证 CAS 更新窗口终点，响应丢失只重放确认。
- 证据缺口、未读完详情、显式业务未完成事项、未决分析、非法 exit batch 均保持 partial；不自动重设历史水位。
- 审查修复：已处置为 closed 的零 turn 分析 Worker 不再永久 pending；反思临时游标不淘汰普通会话游标；Manager 证据读取包含被内存淘汰的磁盘归档。失败清点不发布半成品目录，已发布目录的详情只按原定位和上界重试。已移除协议外的精确 inbox 调用前置，本轮 121 项相关回归与 Agent 构建通过。
- 重启续办保留 daily 身份，避免下一周期把反思自身纳入取证；真实检查点、TraceStore 和取证目录回归先红后绿，本轮 41 项相关回归及 Agent 构建通过。
- 按用户要求直接修订原 spec，取消新增失败账本及逐笔清零门槛；业务完成由主控通过既有结果报告，错误继续保留在工具结果和 trace。真实工具面/Engine/TraceStore 的错误保留与摘要重试、旧候选失败状态恢复回归先红后绿，166 项相关验证及 Agent 构建通过；未新增失败处置接口。
- 已合入 main 并解决工具装配冲突，确认失败的 trace 产品结果改为 partial，保留原确认重发。此前 Agent 505 项定向验证、Agent/Admin 构建通过；Core 172 项、Shared 111 项、CLI 193 项全量通过。Agent/Admin 全量中的旧夹具断言及旧重试测试挂起已与 main 对照，不宣称全量通过。
- 两个任务已按最终协议共同核对；专用正文、定时指令与收尾提醒对齐固定周期取证、等待续办和独立完成提交，补齐新结论进入 inbox、增量建链保留旧边和宿主退出后校验语义。不恢复 Skill、失败台账或精确 inbox 调用门槛。发布验证与剩余行为验收见 `crabot-agent/eval/guidance/DAILY-RELEASE-20260918.md`；历史缺口不自动跳过。

### 百炼空 assistant 消息 400：已修复并部署

- 无工具调用的空回复、纯 reasoning 消息保留 `content: ""`，有工具调用时保持原有 null 与结果配对；不清理持久化会话或修改模型配置。
- 三项回归先失败后通过，适配器 66 项与 Agent 类型检查通过；全模块及前端构建完成，重启后 MM／Admin／Agent 健康。原问题会话离线投影的无工具 null 从 35 条降至 0 条，未向 Provider 重放私有历史。
- 独立 follow-up：SSE 错误体解析及 `invalid_parameter_error` 覆盖 type 导致无效重试；Admin 启动复用初始化旧 HTTP 连接导致注册 ECONNRESET，本次仅用临时启动脚本为注册新建连接，收到响应后恢复默认传输，后续普通重启仍可能复现。

### 群聊 Agent CLI 凭据签发：已修复，待部署

- 按已确认群级统一权限契约，仅私聊凭据要求创建者存在于好友表；群聊创建者保留为审计来源，未登记或已删除的好友不再阻断 Worker 启动。
- 先复现群聊签发的两项失败，再验证群聊签发／实时撤权、私聊拒绝、runtime 身份校验及既有群权限和 Admin Chat 回归，共 28 项通过；Admin 类型检查通过。未修改旧 Worker 权限快照，未部署。

### 微信视频号卡片元信息：已实现，待部署验收

- WeChatCommonJar 从卡片 XML 提取作者、描述、媒体数量、时长和尺寸；connector 保存并透传，微信渠道在实时入站、单条及历史查询中统一呈现文字，明确视频画面和音频尚未读取。旧消息降级为视频号卡片。
- 上游 JVM 解析与序列化 5 项、connector 入库/推送/查询等 34 项、微信渠道 95 项测试通过；共享类型构建、connector 类型检查及渠道构建通过。Android 完整构建需在具备微信类库与 SDK 的机器完成；尚未部署或验证新卡片进入真实 Agent。
...
- Manager 回复统计限定本 episode，并保留压缩重试与重启续跑的当前回合证据；真实错误进入 trace 和 Channel，通知先脱敏再截断。
- 连接恢复经共享 Engine 进度链路进入 trace 及会话通知，沿用五分钟冷却；Admin 进度为独立 delivery，不消费正式回复 request_ids，也不清除待回复义务。
- Agent 首轮核心 282 项、入站／重启 110 项通过；同步最新 main 后定向复验 391 项通过，新增的提示词文案断言失配已在 main 复现，加上原有技能夹具及旧压缩恢复用例，共 11 个基线问题。飞书全量 279 项及两模块构建通过。8 月 26 日私聊只读实测：本地该日 0 条，新处理器用一次详情解析、两页查询取得 60 条，身份不变且未回填缓存。未部署；重复无效查询治理、max_turns 状态语义另行处理。
...
- 审查修复主线失败误杀后台 Shell、`query_worker` 漏返稳定分支 ID，以及 Claude 分支遗漏主线已有的 auto 启动参数。真实生产装配覆盖 Shell 保活后接续、按返回 ID 读取分支证据；原生 CLI 本机夹具验证 Claude/Codex 分支实际写出 42，Claude 定向 12 项回归和类型检查通过。保留终端 seq 兼容，Codex 网络继承差异及历史 receipt/事件字段漂移另记 follow-up。

### Worker 执行行为与项目文档维护：用户确认合入 main 上线观察

- 依据 bot-2 最近三天的消息及 builtin 原生记录，调整无依据设限、局部任务过度分工、失败后的有效推进和按真实结果验收；不以降低拒绝率、工具调用数或测试数量作为成功标准。
- Manager 对持续无效的复用请求停止，确认停止后另起新 Worker；同步修正新建工具说明中的强制复用倾向，以及停止工具说明的旧状态名。没有改变工具接口、权限或状态机。
- 9 月 13 日按反馈删除具体报错及 stdin 诊断特例，保留通用证据与阻塞判断。Manager 使用现有 splitManagerKey 在代码中拆分，再将 channel_id/session_id 作为 JSON 注入，不要求模型解析复合标识。
- 已移除主控决策文档写入口、更新共享 Skill 和压缩规则、修复飞书 history limit 漏传及 Skill frontmatter 解析。定向回归与三模块类型检查通过，未改变权限、状态机或业务项目现场。
- 固定初版 72 个轨迹槽位，修订开发 24、已见案例复查 6；结束不等于通过。保留集发现缺材料却标完成，已用一次通用修订纠正；原评测器存在状态和回执缺陷，Manager 多步验收及盲评未完成，不能宣称泛化改善。
- 后续修复评测器生产工具回执、独立 Worker 状态及逐请求留档，15 项回归通过；同候选追加 12+3 个样本、123 次请求均有记录，模拟纠偏/换新已有推进证据，仍有收尾错误与覆盖缺口。未调生产提示词，未计为整体通过，详见 `crabot-agent/eval/worker-behavior/EVALUATOR-REPAIR.md`。
- 实际运行与文档纠错有产物证据；最终 Skill 已在 Claude 原生读取。修订 builtin 三阶段截止仍 running，Codex 最终包装版实际行为未完整复验，整体离线行为验收未通过。测试 Worker 已停止；完整证据及失败见 `crabot-agent/eval/worker-behavior/RESULTS.md`。
- 9 月 14 日接手复验：修正 Skill 正文断言后，Agent 定向集合各文件最新结果 252 项、飞书 26、Admin Skill 3、评测器 17 项及三模块类型检查通过。三个新 builtin 实跑中，库存汇总与脚本最小修复完成并收口；缺材料场景因上游 502 未验证，范围冗余、Manager 完整链和最终 Codex 仍待验收。候选提示词未改。
- 9 月 14 日用户明确要求直接合入 main 并上线观察，覆盖原先等待完整离线验收和 PR 发布的安排；不改候选提示词，不把发布记为行为验收通过。
- 独立 follow-up：Telegram history 漏传 limit；钉钉分页优先级及正整数校验、微信正整数校验未对齐。本次只修飞书，不扩围。

### 重试切换模型的后续轮次：已修复，未部署

- 共享 Engine 保留重试已接纳的 adapter/model/max_tokens/thinking，执行工具后继续使用新配置；正常调用的快照边界及调用方配置对象不变。
- 新增 4 项回归，3 项先复现旧 Provider 被再次调用；相关定向验证 126 项通过，Agent 构建与 diff review 通过。额外整文件重试测试挂起、测试文件两处类型错误均在未修改 main 复现，未扩围处理。

### Manager 回合来源展示：前端修复完成，未部署

- 历史活动明确区分管理回合、本轮起因、本轮回复与操作，避免把对插话的回复误标为自省来源；原始 trigger 和查询接口不变。
- 22 项定向测试、前端构建、桌面和手机交互验证通过。已结束回合的插话原文补齐仍需后端历史查询支持，本次未扩展。

### 主控与执行器提示词：全文精简已实施，未部署

- 用户确认完整 diff 并要求直接提交推送 main。明确主控负责会话背景、执行器负责自主执行，保留既有授权、并行与分支、事件处置和验收规则；没有调整工具权限、接口或状态机。
- 合并重复的判断与排障要求，删除正文中的工具参数教程、重复子 Agent 清单和 Skill 加载说明；保留真实工具描述及动态 Skill/项目内容。修正 Output 的过时工具指引、finish_task 的回合/任务混淆和子 Agent 状态销毁措辞。
...
### 主控与执行器内置 Guidance：增量已发布部署，线上行为待观察

- 补齐内置 Skill 退役：删除仍被 Admin 注册的 `memory-graph-linking` 载荷，独立图谱重建已内联规则；每日反思遗漏的增量规则在本轮专用正文中补齐，不再维护独立 Skill。68 项注册/快照回归通过，覆盖旧内置项清理、同名用户 Skill 保留及重启后的持久化；不改变图谱规则、权限或业务执行入口。
- 已确认两个核心与七份工作流归位：任务板概念常驻核心，自省和执行器反馈按场景加载；普通改板直接执行，管理员更新仅带短提示。保持权限、缓存、每日反思及子 Agent 隔离契约；该批正文基线为 ea1103f1；本轮另应用已核对的等待、搜索及每日反思补漏。
- 定向回归、Docker 真实工具链与实际模型对照已完成。真实事故历史六对旧版外发 2 次、候选 0 次；连续五轮旧版外发 3 次、候选 0 次。必要提问、正常交付及自省主动恢复取得实际结果；一条恢复轨迹交付后被预算截断，连续候选三轮未重新查板，限制与全量成本保留在 `crabot-agent/eval/guidance/IDLE-REPLAY-RESULTS-20260918.md`。
- 两条权限反例已核对真实输入：核心和工具说明均明确先核实、缺权限先询问，且 can_spawn=false 已进入模型输入；模型仍误要已有能力或尝试被拒派发。未发现规则漏装或权限门禁漏洞，零 Worker/文件副作用；保留行为失败，不继续叠加同义提示词或宣称整体无回退。
- 按用户指定路线直接发布 main；同步已合并搜索修复后的 330 项定向回归、12 项 Docker 场景及 Agent 构建通过。2026-09-18 15:25 已定向重启本地 Agent，九份编译正文与确认稿一致，健康检查及启动对账完成。尚无部署后的真实自省样本，不将部署成功当作线上行为验收。本轮等待/搜索与每日反思补漏的发布验证另记；完整历史报告和未通过项均保留。
...
- 在上述候选基础上，生产提示词加入主控与执行器的双向信息消化规则：不做传话筒，向执行器直接委托，收到执行器授权请求时结合已有意图和边界自行判断；阻碍人类需求的授权行为不再反复向人类索取，由主控代人类向执行器授权。私有评测材料仍仅保留在本地忽略目录。
...
- 按确认稿装配 builtin 专属提示词，删除不可用的消息、通讯录、旧任务工具及反思指引，
  保留既有委派、核验、研究持续性与 finish_task 约定；进展、结论、证据和问题清晰说明。
- 完整渲染与确认稿逐字一致，16 种通用 Agent 装配组合保持一致；生产入口覆盖人格、Skill、
  workspace 快照及有无子 Agent，相关执行器回归和 TypeScript 检查已运行。
- 定向测试中的旧 Goal 标题与 ended_reason 断言失败在未修改基线同样复现，未扩围处理。
- 已对齐 writing-plans 的规划交付边界，删除额外执行流程及强制 Skill 依赖；验证 Skill 按当前改动
  和结论范围判断证据有效性，允许复用仍有效的结果，正文与列表简介一致。
- Skill 修复逐字匹配确认稿；注册与子 Agent 绑定等 45 项定向测试通过。
...
- 合并最新 main 后的 Agent 全量测试：3034 通过、63 跳过、23 失败；其中 20 个失败在 main
  基线复现，其余 3 个单独复跑通过。完整对照记录见文档仓实施计划，本次不扩围修复已有测试问题。

### Claude Code 工作目录字符转换：已修复

- 原生会话目录定位对齐 CC 的字符转换规则，将下划线、空格等非 ASCII 字母或数字字符替换为 `-`，
  恢复这些工作目录的 trace、原生活动监听及相关会话读取。
- 固定目录名回归覆盖 trace/cursor、活性时间和追加通知；相关定向测试与 Agent 构建通过。
...
### Manager 任务板分层、稳定寻址与空闲续办：主体已合入（PR #149），v3 首读迁移待 PR 审查

- 一会话一板保持 Objective/WorkItem 两层：目标记录结果与完成条件，一个目标可有多项当前事项；事项只保留
  状态、可选项目范围、当前判断、下一步和一个主要阻塞。完成或放弃后进入 archive 最终快照，不增加优先级、
  Worker 绑定、决策引用或历史恢复。
- Store 升至 schema v4，为目标和事项生成不可变的 `objective_id` / `work_item_id`，只供 Manager 工具、Admin
  API 和页面技术寻址；标题仍负责语义判断和人类呈现。重命名、移动后 ID 不变，写入目标已过期时明确失败，
...
- 合并最新 main 后，Agent 定向 220/220、Admin API 3/3、任务板前端 11/11、三包 TypeScript 与前端生产构建通过；
  Docker 确定性评测 20/20（25 次请求），`mirror-xinshu / gpt-5.6-terra` 真实评测 33/33（六轮、12 次请求）。
  全量测试 3182/3200；5 项并发清理失败串行复跑通过，剩余 13 项在干净 `origin/main` 上逐项复现。
- Store 补齐唯一前序 schema v3 的首次读取迁移：在会话互斥锁内补 UUID、按 fence 最后位置映射实体，完成
  v4 全量校验后原子写回；revision 与 notice 不变，非法、歧义或写回失败时保留原文件。相关 116 项测试与
  Agent TypeScript 构建通过；本机三份存量任务板副本迁移成功，revision 不变且二次读取稳定。
