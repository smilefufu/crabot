...
describe('getBuiltinSubAgents', () => {
  ...
  it('task_reviewer is the default combined task review gate', () => {
    ...
    expect(r.deliverables).toContain('若 STATUS=CANNOT_VERIFY，则 spec_compliance.verdict 必须为 CANNOT_VERIFY，且 assessment 必须为 NEEDS_FIX')
    ...
    expect(r.verification).toContain('若 STATUS=CANNOT_VERIFY，spec_compliance.verdict 是否也是 CANNOT_VERIFY，且 assessment=NEEDS_FIX？')
