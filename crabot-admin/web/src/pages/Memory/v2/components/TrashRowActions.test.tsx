...
describe('TrashRowActions — 30-day restore window (spec §6.5)', () => {
  it('renders enabled Restore button when entry is 1 day old', () => {
    ...
    render(<TrashRowActions trashedAt="2026-04-23T00:00:00Z" now={() => NOW} onRestore={onRestore} />)
  ...
  it('renders disabled button when entry is exactly 30 days old (boundary)', () => {
    const onRestore = vi.fn()
    render(<TrashRowActions trashedAt="2026-03-25T00:00:00Z" now={() => NOW} onRestore={onRestore} />)
    const btn = screen.getByRole('button', { name: /restore/i }) as HTMLButtonElement
  ...
  it('renders disabled button with tooltip when entry is 35 days old', () => {
    const onRestore = vi.fn()
    render(<TrashRowActions trashedAt="2026-03-20T00:00:00Z" now={() => NOW} onRestore={onRestore} />)
    const btn = screen.getByRole('button', { name: /restore/i }) as HTMLButtonElement
    expect(btn.disabled).toBe(true)
  ...
  it('clicking enabled button invokes onRestore', () => {
    const onRestore = vi.fn()
    render(<TrashRowActions trashedAt="2026-04-22T00:00:00Z" now={() => NOW} onRestore={onRestore} />)
  ...
  it('clicking disabled button does NOT invoke onRestore', () => {
    const onRestore = vi.fn()
    render(<TrashRowActions trashedAt="2026-01-01T00:00:00Z" now={() => NOW} onRestore={onRestore} />)
    fireEvent.click(screen.getByRole('button', { name: /restore/i }))
  ...
  it('respects custom retentionDays prop', () => {
    ...
    render(<TrashRowActions trashedAt="2026-04-17T00:00:00Z" retentionDays={7} now={() => NOW} onRestore={onRestore} />)
    const btn = screen.getByRole('button', { name: /restore/i }) as HTMLButtonElement
    expect(btn.disabled).toBe(true)
  ...
  it('treats invalid ingestion_time as expired (defensive)', () => {
    const onRestore = vi.fn()
    render(<TrashRowActions trashedAt="not-a-date" now={() => NOW} onRestore={onRestore} />)
    const btn = screen.getByRole('button', { name: /restore/i }) as HTMLButtonElement
    expect(btn.disabled).toBe(true)
  })
})
