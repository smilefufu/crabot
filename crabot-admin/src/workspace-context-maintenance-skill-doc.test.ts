...
import { SkillManager, parseSkillMd } from './mcp-skill-manager.js'

describe('workspace-context-maintenance skill doc', () => {
  it('随任务维护项目文档，区分事实与规范并保留 Git 纪律', () => {
    ...
    expect(doc).toContain('实现事实依据代码和验证结果，规范性要求依据项目已成立的决定')
    ...
    expect(doc).toContain('包括一般文档与决策记录')
    expect(doc).toContain('可写 ADR 不等于有权创设政策')
    expect(doc).not.toContain('不创建、修改、取代或迁移决策记录')
  ...
  it.each([false, true])('新旧注册表均向 Agent 提供已批准的初始化简介（existing=%s）', async (existing) => {
    ...
    try {
      ...
      expect(parseSkillMd(doc)).toMatchObject({ name: current.name, description: current.description })
      expect(registered.version).toBe('4.0.0-crabot')
