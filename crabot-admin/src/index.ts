    ...
    const SEEDS: Array<Pick<Schedule, 'name' | 'description' | 'trigger' | 'task_template'>> = [
      {
        ...
        task_template: {
          ...
          title: '每日反思 — {{date}}',
          description: '这是主控直接执行的每日反思，不调用 Skill，也不把记忆写入委派给执行器。这是一次唤醒，不代表开始新周期；先用 list_reflection_records 核对固定周期与持久进度，未完成则续办，不能用本次触发时间替换旧周期。标准流程：1）用 list_reflection_records 翻完本周期目录，用 read_reflection_record 读完所选记录，筛选失败、异常轮数、负面反馈和可复用成功经验；可委派普通执行器分析单条 trace，但它只返回结论；有价值的新结论带 task/trace 来源写入 inbox 并在本轮核验，不强行产出记忆；2）用 list_entries(status:"inbox",sort:"ingestion_time_asc") 读取待审候选；正常候选逐项核验，缺少 inbox_entered_at 的历史候选保留原有人工确认迁移流程，不在本轮自动重判或清理；3）有价值 fact/concept 先做 PE 比对再 promote_inbox_entry，lesson case 也用 promote_inbox_entry；4）重复、无价值或证据不足候选用 delete_memory 移 trash，不把“留待下次”作为常规结果；5）同 scenario、同 outcome 的至少三条 confirmed lesson case 用 promote_to_rule，source case 保持 confirmed 但 maturity=retired；6）近期新增、变更及本轮召回涉及的 confirmed 记忆按专用正文增量建链，先读取并保留旧 links，无新增不清空；7）先完成当前可推进事项，局部缺口不阻止其他可做工作；只剩真实阻塞或取证缺口时才以 partial 收口，全部条件满足才报 completed。单独调用 finish_daily_reflection，按真实结果填写 outcome、pending_items 和 evidence_refs；提交后由宿主校验并确认水位，不把退出标记当完成证明；等待分析结果且暂无可推进事项时结束本轮等通知，不把等待当作完成。不得调用 run_maintenance(all)；机械维护只由凌晨 04:00 的 memory_maintenance 执行。只在有符合既有阈值的人类可读发现时调用 send_daily_reflection_summary 一句摘要；它只投递到 Admin Web 系统任务线程，禁止外发 trace、Evolution Mode 或数字明细。',
          priority: 'low',
